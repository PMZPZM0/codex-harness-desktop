...
console.log("remote port:", port);
const before = await evalJs(`({ events: document.querySelectorAll(".system-event").length, threadTitle: document.querySelector(".task-title strong")?.innerText })`);
...
const afterConnect = await evalJs(`({
  ...
  threadTitle: document.querySelector(".task-title strong")?.innerText,
})`);
sock.send(JSON.stringify({ type: "command", command: "/status" }));
await sleep(2000);
const afterCmd = await evalJs(`({ events: document.querySelectorAll(".system-event").length, lastEvent2: [...document.querySelectorAll(".system-event")].at(-1)?.innerText.replace(/\\s+/g, " ").slice(0, 60) ?? "" })`);
sock.close();
