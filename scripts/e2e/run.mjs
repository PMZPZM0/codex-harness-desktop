...
//
// 用法：
//   npm run e2e                  # 跑默认场景（smoke）
//   npm run e2e -- smoke         # 指定场景
...
const h = new ElectronHarness({ root: ROOT, artifactsDir: join(ROOT, ".e2e-artifacts") });
const t0 = Date.now();
const results = [];
...
console.log(`\n总耗时 ${((Date.now() - t0) / 1000).toFixed(1)}s，截图目录：.e2e-artifacts/shots\n`);

process.exit(checkSummary.ok && badSteps.length === 0 ? 0 : 1);
