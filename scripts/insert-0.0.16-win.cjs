...
const CHANGELOG = ["【新功能】专家团会话侧栏聚簇：主会话 + 成员会话合并为一行（「N 会话」徽标），默认折叠，点箭头展开「主会话/成员会话」分节（限高滚动适配）","【修复】欢迎页项目地址流程：选完目录不再丢消息、不再建进错误的临时目录","【优化】项目地址菜单三段式：历史项目地址（直接沿用）/ 新项目地址 / 不使用项目地址，选项加大","【修复】成员工作弹窗：运行中关掉再点开历史不再空白（活跃委托并入历史）、产出自动跟随最新、滚动不再穿透主消息区","【修复】成员/子智能体首条消息不再裸露角色提示词（旧会话打开即正确折叠）","【优化】成员头像轨改浮层：消息区不再被右侧空白列挤压，气泡与输入框右缘对齐"].join(String.fromCharCode(10));
