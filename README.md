...
> ⚠️ **磁盘空间**：首次启动会按需下载运行时与工具链（Node / Git / 引擎 / 浏览器内核等），**建议至少留 10 GB 可用空间**。只剩 1~2 GB 时的典型症状是：解压出来的 App 不完整、更新下载失败、工具装到一半报错 —— 这些都不是应用本身的 bug，先腾空间再排查。
...
1. 解压后把 App **拖进「应用程序」**（别在「下载」里直接双击 —— 那样系统会把它挂在只读的临时卷上运行，功能会异常）
2. **右键 → 打开**（不要直接双击），弹窗中点「打开」
3. 若仍提示「已损坏」/「无法验证开发者」，在终端执行：
