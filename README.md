...
- **发送锚顶（09-12 傍晚，对齐 WorkBuddy 观感）**：发送消息后不再贴底等回复，而是**把这条新消息钉在对话区顶部**、回复向下方的空白处流式展开——第一屏上方内容纹丝不动，消灭「正文出字上下跳动」（旧贴底模式每个字都把整屏往上顶、占位头塌陷时再猛坠）；回复长满第一屏后自动无缝转回贴底跟随；用户向上滚动立即解除钉顶自由翻阅。新会话首条消息的欢迎块中间态、真实回合接管时机等边界都已自动处理。回归：`scripts/e2e/scenarios/send-anchor-top.mjs`（12 断言，真实引擎发长消息）。
