...
- **基础运行时**：Node/VS Code CLI 随应用内置；Python/Git/PowerShell/rg/uv/CMake/7-Zip/jq/Ninja 在「开发工具」页按需下载（国内镜像优先，失败回落官方源），**首次启动检测到缺 Git 会自动后台安装**
...
> 基础运行时不再整体内置（09-16 瘦身）：仅 Node（安装器引导）与 VS Code CLI 随包，其余在「开发工具」页按需下载——npmmirror/gh 国内加速优先、失败自动回落官方源，**缺 Git 时首次启动自动后台补装**；桌面/浏览器自动化工具与浏览器内核同样按需下载，不再随包塞 1.2GB 内核。
