...
- **能读文档附件（09-21）**：把 PDF / Word / Excel / PowerPoint 拖进对话，模型读到的是**转成 Markdown 的正文**（内置 markitdown，MIT 许可），不再只拿到一个文件路径；它装在 Python 里、随 Python 一起装好，老版本升级后若缺可在「设置 → 开发工具 → 文档转换（markitdown）」一键补装（约 120 MB，清华 pip 源）
