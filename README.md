...
> ```
>
> **只跑一半不算验收**；改了哪个模块，就给 `scripts/check-preflight.mjs` 加**可保留的检查项**（静态守卫 + 纯函数断言，毫秒级、不起应用，让这次验证沉淀成下次的自动回归）；需要真链路证据的场景**每轮现写、跑完即删**——不往常驻套件里沉淀、也不重复跑已经验过的流程（用户 09-14 口径）。**不做反证**（用户 09-19 明令：「把你的反证流程删了，以后不需要再反证了」），UI 改动必须看截图。
