...
- **发送锚顶（09-12 傍晚，部分完成）**：发送消息后新消息**钉在对话区顶部**（WorkBuddy 观感），回复向下方的空白处流式展开，视口全程稳定——不做自动转贴底（旧贴底模式每个字都把整屏往上顶 = 「出字上下跳动」）；长回复超屏后点「回到底部」或自己滚到底即恢复实时跟随；向上滚动随时解除钉顶翻阅历史。第一条消息已实测全程稳定；**遗留**：连续发送时第二/三条起钉顶会失效退回旧行为（排查线索与工具已记录在 AGENTS.md「发送锚顶」条目与 `window.__adbg` 探针）。回归场景：`scripts/e2e/scenarios/send-anchor-top.mjs`。
