...
| 🖥️ | **桌面与浏览器自动化** | 键鼠/截屏/OCR/窗口控制 + Playwright 与反检测指纹浏览器；Node 随包内置，其余工具链与浏览器内核在「开发工具」按需下载（npmmirror/gh 国内加速，失败自动回落官方源） |
...
- **桌面与浏览器自动化可安装（09-12 修复）**：随包 automation-tools.zip 由打包钩子在打包前自动生成（此前 Windows 包漏带该文件，点安装必报「缺少 automation-tools.zip」）；zip 缺失时还会自动回落 GitHub Release 在线下载。安装包因此约 +18MB。
