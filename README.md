...
- **引用体系**：`@` 引用上下文、`#` 引用技能（与 `/` 命令面板同款：输入 `#` 弹出技能面板，等宽技能名 + 一句中文注释，回车或点击即引用；未匹配时按普通文本发送）；**输入框技能预提醒**——空输入框时顶部提示「输入 # 可引用技能（N 个可用，支持中文搜索）」，可一键展开技能列表，点「不再提示」后长期关闭；引用会话/文件、图片内联粘贴；**输入框直接粘贴文件**（系统复制的 PDF/代码/文档等 Ctrl+V 即作为附件，多选批量粘贴，发送时以附件清单带上）；**图片查看灯箱**：点击消息/附件图片放大（滚轮缩放/拖拽平移），工具栏支持**复制图片**（本地图片经主进程写入剪贴板，网络图直取）与**打开所在位置**（本地图在文件管理器定位、网络图用浏览器打开）
