...
- **模型是会话级设置，且「谁后改谁生效」**：引擎的 `thread/resume` 不回带模型，模型由客户端在每轮 `turn/start` 里下发——所以每个会话都记得自己的模型。打开会话时按**用户显式选择的时间戳**判定：全局默认比该会话的记录更晚被改过 → 用全局（改了全局模型打开旧会话即生效）；会话里后改过 → 用会话自己选的。只有**手动选模型**才计入时间戳，自动回填/新建会话不算（否则回填会把自己刷成「最新」，全局永远进不来）
