...
### ✅ 关键词模型（KWS）已内置（用户 09-13：「安排一下，内置好」）

上面那条「未做」项已做掉：唤醒现在**优先走专用关键词模型**，识别模型只作回退。

| 项 | 内容 |
|---|---|
| 模型 | `sherpa-onnx-kws-zipformer-wenetspeech-3.3M-2024-01-01`（GitHub release 归档 31.1MB，SHA256 `b2f7c89…7f35f`）；用 float32 `epoch-12-avg-2` 三件套（3.3M 参数，**不是** 154MB 识别模型） |
| 安装 | 归档型按需安装：多镜像下载（实测直连 GitHub 失败 → `ghfast.top` 前缀成功，55.7s）→ SHA256 → 随包 Python 解压 → `kwsReady` 校验 → 清归档；**幂等复调 0ms**。入口：设置 → 语音通话 → 唤醒卡片「下载唤醒模型（约 31MB，推荐）」；IPC `voice:kws-install/cancel/status` |
| 关键词生成 | 新增 `electron/voice/kws-keywords.ts`（纯逻辑）：**注音（音色词典 lexicon.txt）→ 带声调拼音 → 拆声母/韵母 → 按模型 token 表校验**。生成器**必须复现模型自带的 8 行 `keywords.txt`** —— 预检【4e】逐行对照，实测 **8/8 完全一致**；全 lexicon 20892 字：转换成功 **100%**、落在模型 token 表内 **99.7%** |
| 唤醒效果（真模型实测） | 说「小柯小柯」→ **命中**；说同音变体「小科小科」→ **命中**（KWS 按读音匹配，不再需要同音兜底）；3 句日常话 → **零误触发**；跨关键词（小米小米）→ 不触发。启动耗时 **1.0s**（识别模型路径是 6.5s），常驻 CPU 也低一个量级 |
| 回退 | 没装关键词模型 / 唤醒词转不出拼音 token（生僻字）→ 自动回退「识别模型 + 同音容错」，并在卡片上说明原因与下载入口 |
| 不改动的东西 | KWS **不进** `ALL_VOICE_REPOS`：它只服务唤醒，没装不该把通话/听写判成「模型未下载完整」 |

**端到端复验**（跑编译产物 `VoiceService` + 真 KWS 模型）：`engine=kws`，`keywords.txt` = `x iǎo k ē x iǎo k ē @小柯小柯`；
唤醒词命中 ✓、同音变体命中 ✓、三句干扰句全不误触发 ✓；**改唤醒词后自动重建 worker**：新词「小助手」命中 ✓、旧词不再命中 ✓。
**反证**：去掉「j/q/x 后 ü 写成 u」→ 模型样例 2 行不一致 + 专项断言红；去掉舌尖元音特例 → 转换失败断言红；
把 KWS 分支条件改成 false → 「优先用关键词模型」守卫红；worker 去掉命中即 reset → reset 守卫红。
