# 功能变更轮次记录（CHANGELOG-ROUNDS）

> 本文件由 AGENTS.md 拆骨迁出（2026-09-24，评估报告 P0-2）：AGENTS.md 曾达 503,805 字节，
> 被 harness 截断到 65KB（模型只读到 13%）——历史小节整体移到这里，**不再回迁**。
> 本文件不进引擎注入，仅供人查；现行规则只看 AGENTS.md。
...
- **增强按钮提示气泡（09-17 加，09-20 按用户「弹出来频率太高，一输入文字就出来」重定口径）**：
  输入框右侧的 AI 优化按钮（`.enhance-button`，紫色星星）上方浮出小气泡，文案取自
  `src/lib/enhance-hints.mjs` 的词库（随机且不与上一条重复；09-20 起**全部 ≤16 字**，小气泡里才不挤成块）。
  **⛔ 当前口径（以此为准）**：① **第 1 次成功发送后**弹一次 ② 之后**每 10 次**成功发送弹一次
  ③ 输入长需求（**≥120 字**）弹一次；冷却 **3 分钟**、气泡停留 **3 秒**；点气泡本体=开始优化，
  点右侧 ✕=只关提示（`stopPropagation`），点增强按钮也消费掉。
  **⛔ 旧的「每次启动后第一次**输入**必弹」已彻底删除**（那才是"一输入文字就出来"的真凶）：
  连同 `shouldShowHintThisRun` / `markHintShownThisRun` / `shownThisRun` 这套模块级不落盘写法一起移除，
  预检【32】专门留了一条**「旧机制只要还在就判红」**防复发。
  **⛔ 打字期间一次都不弹**：展示只由「发送后 pending」+「长输入」驱动，而气泡依附的按钮只在
  输入框有文字时渲染 ⇒ 「第 1 次发送后」的提醒**实际是下次打字时才看到**（结构上做不到"发送瞬间弹"）。
  **⛔ 条件③必须双向节流**：按"编辑会话"节流（输入清空才重置 `enhanceLongFiredRef`），
  且触发共享 `HINT_COOLDOWN_MS` 冷却窗口，否则边打字边弹/刚弹过又弹 = 骚扰。
  **⛔ 两个真坑（实测踩出来的）**：
  ...
  **⛔ 增强的"取消"与"还原"**（用户 09-17 明确要求"不满意要能取消增强、返回原输入"）：
  ...
  预检【32】⑰ 现按 09-20 新口径重写（含"旧机制必须不存在"这条防复发；历史记录：09-17 那版曾是
  十三条守卫 + 10 条变异反证，反证过程暴露并加强了 3 条太弱的守卫 —— 长输入阈值要测**边界**
  （阈值 09-20 改为 120 ⇒ 边界是 119/120）、节流要测**耦合表达式**、取消令牌要锚在**请求之后紧跟的位置**，
  只查文本存在会被同名的另一处顶成假绿）。CDP 真启动验收 12 条曾全绿（真发一条消息走完整链路）。

- **输入框下方的 AI 内容提示（09-20 用户：「在输入框下面空白居中位置加一排字：内容由AI生成，请核实重要信息」）**：
  `.composer-wrap` 的**最后一个子节点** `.composer-disclaimer`（`<p>`，静态一行，11px / `var(--faint)` /
  `text-align:center`），所以它随输入框那一列居中（不是相对窗口 —— 右侧面板开合时不会偏）。
  正常态落在输入框下方 8px、距窗口底 22px；欢迎页（`docked-center`）落在建议 chips 下方。
  **⛔ 两个必须**：① **静态、高度常量** —— `.composer-wrap` 上挂着 ResizeObserver（高度一变就重申贴底），
  条件渲染或动态高度会让它反复触发（表现为贴底抖动）；② **不脱流**（别用 absolute/fixed），
  否则它不参与该列高度、居中基准就不是输入框那一列了。预检【77】5 条守卫钉住这三点 + 文案原文。

- **会话改名：名字必须由**本地覆盖**权威（09-20 用户：「导入会话，改左侧栏名字不生效，一直显示导入…」）**：
  实测根因（隔离 CODEX_HOME + 真实 app-server，两条都硬取证）：
  ① **引擎不回带我们设的名字** —— `thread/name/set` 返回 `{}` 不报错，但 `thread/list` 里的 `name`
     仍是引擎**按该会话首条用户消息自动生成**的那个（改完立刻读也没变）。会话一旦有了消息，
     引擎列表的名字就跟消息走；「导入会话记录」的首条消息正是整段导入记录 ⇒ 永远显示「导入…」。
  ② **旧 `renameThread` 的乐观更新被同一拍抹掉** —— 先本地改，紧接着 `refreshThreads()` 用引擎那份
     **整表替换**，改动当场消失（观感 = 改名一点反应都没有）。
  ⇒ 修法与既有 `cwdOverrides` **同构**：`thread-name-override-v1`（localStorage）+ `rememberThreadName`
  / `effectiveThreadName` / `withNameOverride`，并在**所有引擎回包落地处**贴回：两处 `thread/list`
  入口、`freshThread`（新建/导入）、resume 回包（漏一处「刷新一下名字就弹回去」）。
  `thread/name/set` 保留为**尽力同步**（别的客户端还看它），失败不回滚本地改名，提示措辞是
  「已改名（引擎侧未同步…）」而**不是**「重命名失败」（本地已生效，说失败会让用户以为白改）。
  ⚠️ 顺带查实：**无回合的会话不在 `thread/list` 里**（`has_user_event=0` 一条都不返回，resume 也没用）
  ⇒ 「导入会话记录」建出的空会话在发出第一条消息前**不会出现在侧栏**（`importConversationMarkdown`
  里「刷新一次就立刻出现在左侧」那句注释与当前引擎行为不符）。预检【78】7 条守卫。

- **折叠头不再显示「N 项失败」文字（09-20 用户明令：「把那个折叠线上后面的那个几项失败文字删了，
  这个不要」）**：`.wb-fold-header` 里那枚 `{failedCount} 项失败`（`FoldGroup` 是所有折叠头的唯一
  渲染点，删一处即全生效）删掉，配套样式 `.wb-fold-failed` 一并清除（不留死代码）。
  ⛔ **「这轮有失败」的表达仍保留在左侧小圆点**（`.wb-fold-dot.error`）—— 删文字 ≠ 删信号，
  预检【79】把这两件事一起钉住（文字不许回来 + 圆点不许被顺手删掉）。
  ⚠️ 已知副作用：失败信息现在只剩视觉圆点（该圆点本就是 `aria-hidden`），折叠头的可访问名称里
  不再包含失败数；`failedCount` 仍作为 prop 传入（只喂圆点）。

- **折叠头不再显示右侧概要统计小字 + 箭头改到内容后面（09-23 用户附截图：「这些文字多余，
  全部删掉，展开折叠图标动态展示在内容后面就行，鼠标放上去展示，不放上去默认隐藏，点击后可以展示」）**：
  ① 删掉 `.wb-fold-stats` 那一列（「调用服务 6 次、查看 4 次、深度思考 7 段」/「深度思考 1 段」），
  `FoldGroup` 的 `stats` prop、`statsTextOf`、四条 `stats={…}` 传参、CSS 规则一并清除（不留死代码）；
  ② `.wb-fold-caret` 由「标题之前」改到**标题之后**（`order: 2`），`opacity` 默认 **0**、只有
  `.wb-fold-header:hover` 才显形（用 opacity 不用 display ⇒ 悬停不产生布局跳动）；
  ③ 完成态标题的 `flex: 1` 改 `0 1 auto`，否则标题撑满会把箭头又推到行尾。
  ⛔ **分类纯函数 `src/lib/tool-call-classify.mjs` 保留**：删的是「展示」，不是分类逻辑
  （预检【112】①②③ 仍真跑它；④ 与【118】反向钉住「不许再挂回折叠头」）。
  ⚠️ 知情代价：**展开前看不到本段干了多少活** —— 这恰是同日早些时候用户明确要过的能力
  （「展开前能看到运行了几条命令」），当天被同一个人推翻，属已知反复，别再"顺手加回来"。
  **灰线：只留回合收尾那一条，正文之间的芯片一条都不许有**（09-23 同日三轮指令，后两轮附完成态截图）：
  运行中是 `.running-process-time`（「正在处理 N 秒」），回合完成后**同一个槽位**换成回合收尾头
  （「耗时 1m 36s」／停止原因）—— **两条都要有** `border-bottom: 1px solid var(--line-soft)`，
  且几何一致（`min-height: 28px` + `padding-bottom: 4px`），否则运行→完成时线会凭空消失。
  ⛔⛔ **但"只有 lead 是 completed"≠"只有 lead 有线"**：`.wb-fold--completed .wb-fold-header` 是
  **后代**选择器 —— 大折叠展开后，体内每个芯片（`.wb-fold--summary`，即「工具折叠」）的折叠头也命中它，
  白得一条线 + `width:100%`（真机实测 `.wb-fold--summary > .wb-fold-header` 得到 `borderBottomWidth=1px`
  且 `width=810`，而 base 是 `width: fit-content`）。用户截图里「正文 / 芯片 / 正文 / 芯片」每段之间那条线
  就是这么来的（「工具的折叠不要灰线，只保留大折叠的灰线」）。
  ⇒ 修法 = 紧跟一条**嵌套归零**规则：`.wb-fold--completed .wb-fold-body .wb-fold-header` +
  `.wb-fold--completed .wb-fold .wb-fold-header`（两个都留：一个按结构、一个按任意嵌套层，
  特异性 (0,3,0) > (0,2,0)，与顺序无关）。**只归零 border，不动 width/min-height/padding**
  （芯片的满宽/28px 行高是用户没抱怨过的形态，改它会引起整段高度变化）。
  预检【122】10 条钉死（含两条嵌套归零断言，防后代泄漏回归）。
  实测（`.e2e-profile/main` 真实多轮回合）：大折叠体内折叠头 21 个 → 带灰线 **0**；回合级头 27 个 → 带灰线 **27**。
...
- **工具下载源选择 + 首启体检去工作区（09-20 两轮用户口径）**：① `app-settings.downloadSource`（`auto`默认 / `mirror` / `ghproxy` / `ghfast` / `direct` / `proxy`），「开发工具」页顶部下拉可切，**runtime:install 每次现读，下一次下载立即生效（无需重启）**。三条通道全认源：工具链 `install-runtimes.cjs` 经 **`DOWNLOAD_SOURCE` 环境变量**（⛔ 不能走 argv——脚本把裸词参数当工具 id）；npm 包（CloakBrowser）direct 只走官方 registry；浏览器内核 direct 只走官方、其余镜像优先（Cloak 的镜像本身就是 ghfast 前缀）。② **auto（默认）= 真国内优先**：镜像 → (代理) → **gh 加速 → 直连兜底**——旧 auto 把直连排在加速前面，而直连不会"失败"只会龟速（PowerShell 7 实测 7% 爬行），加速通道永远轮不到；gh 加速只对 github.com 资产生效，mirror 模式对无 npmmirror 镜像的纯 GitHub 资产也回落 gh 加速打头。③ **首启体检（EnvCheckDialog）移出工作区**（用户定稿「工作区不要必选，就保留工具下载」）：旧版「工作区→去选择」会把弹窗带走（`setEnvCheckOpen(false)` + 跳设置页），用户点了就丢工具安装。现在体检 = 模型 + 工具（必备 4 + 常用 3），一键安装直奔下载。预检【工具下载源】3 条 +【首启体检】2 条 +【32】重锚 3 条。语音模型下载不在范围（本来就只走 npmmirror）。
- **工具安装「验证」非致命（09-20 Miniconda 误报）**：`install-runtimes.cjs` 的 verify 段（git/rg/uv/cmake/conda/gcc/ffmpeg/code 八条）全部走 `safeVerify`（try/catch），单条失败只打 `[verify X] unavailable`，不再被 main() 的 catch 连坐成「安装失败」——用户实测 Miniconda 下载+安装全成功，仅 `[verify conda]` 因 **conda.exe 启动器不支持含空格的安装路径**（应用工具目录必含空格，conda 已知限制）抛错而整体报失败。conda 失败时附空格路径说明。预检【工具安装验证】1 条守卫（禁裸 runCommand verify）。
...
- **账号「生效」不变量（09-21 用户实测「中转站我已停用，他还显示当前生效」→ 查清并修）**：**停用 / 没有凭据 / 不存在的账号永远不是 `activeId`**。判据集中在 `electron/relay-accounts.ts`（纯函数：`isRelayAccountLive` / `pickRelayActiveId` / `normalizeRelayStore` / `relayProviderIdOf`），预检【89】加载编译产物跑真断言。真机事故：`relay-store.json` 的 `activeId` 指向一个 `disabled:true` 账号 ⇒ 卡片同屏出现「使用中 + 已停用 + 当前生效」三个矛盾标记，而 `readRelayAccount()`（余额/套餐/密钥全走它）会拿这个停用账号当生效账号用。**根因（三个破坏入口，都已堵）**：① `relayAuthedFetch()` 的 **401 自动重登**调 `writeRelayAccount()`，后者**无条件**把被写账号设为 `activeId` —— 而中转站页一打开就 `relay:keys-all` **对每个账号**发请求，于是「只是打开页面」就可能把停用账号顶成生效账号（连引擎模型配置一起改）；修法 `writeRelayAccount(a, { activate: false })`。② `relay:logout` / `relay:remove-account` 把 `activeId` 设成 `accounts[0]`（不看停用）→ 改用 `pickRelayActiveId`，且**只有删/停的正好是生效账号才重挑**（不存在的 id 一律不动 `activeId`）。③ `readRelayStore()` 读取时自愈（指向不存在/已停用的 → 置空并落盘；**只置空、不替用户挑**，否则等于偷偷换模型）。
- **账号管理界面（09-21）**：删除入口**可见化**（三处都是带文字 `<Trash2/>删除`：中转站卡片 / 中转站面板 / OpenAI 卡片，从前只有纯图标 + tooltip，用户找不到）+ **二次确认**（`openAppConfirm`，文案写明「会失去什么/不影响什么」）。`relay:logout`（无参、按 `activeId` 删账号的**隐藏删除入口**）整链删除（handler + preload + 调用点，预检【89】盯三处）——它删的是当前生效账号，而面板是给**某一个**账号开的 ⇒ 可能看着 A 的面板删掉 B。**点卡片/「管理」不再自动切换账号**（曾 `if (!a.active) switchAccount(...)`：用户实测「点击管理直接生效了」——看一眼余额会把生效账号换掉、重写模型配置、重启引擎）；为让面板数据自洽，`relay:overview` / `relay:create-key` 增加可选 `id` 参数（面板看谁就读谁、密钥建在谁身上），非生效账号的**激活动作**（使用此套餐 / 使用）禁用并提示「先设为当前」。OpenAI 侧同款：`openai:accounts` 的 `active` 判据加了 `!disabled`，停用 `openai-official` 供应商时同步清 `auth.json`（原先只置 disabled，卡片会同时显示「使用中 + 已停用」）。
- **账号↔供应商联动命名收敛（09-21）**：`relay-<host 首段>` 的计算原先在**三处**各抄一遍（账号开关 / 正向联动 / 反向联动）⇒ 改用 `relayProviderIdOf()` 单一来源。**联动本身未变**（停用账号→禁用同网关供应商；停用供应商→停用同网关账号）。⚠️ 已知限制：规则 `split(".")[0]` 会丢端口 ⇒ 同一主机上的两个中转站（如 `127.0.0.1` 两个端口）会撞同一个供应商 id；现实用例是域名（`api.pptoken.cc` → `relay-pptoken`），未改规则（改会让存量 provider id 失效、打断用户当前模型绑定）。
- **验收方式（09-21，可复用）**：stub `electron` 模块（`Module._load` 劫持 + `app.whenReady` 永不 resolve）→ 加载 `dist-electron/main.js` → 直接调 `ipcMain` handler → **回读磁盘断言**；再起一个本地假网关（`/keys` 首次 401 → 复现自动重登；`/user/profile` 按 Bearer 区分余额）⇒ 30/30 覆盖：自愈落盘、401 补 token 不改 `activeId`（**同时断言 token 真的被改写**，否则是假绿）、停用/删除的供应商联动、删不存在 id 的边界、按 id 读 overview。⚠️ 坑：`app.getPath("userData")` 必须显式映射到临时目录（否则落到 `userData/userData`，读到空账号库，全绿变全红）。
...
### ① 计划可编辑构件（`src/components/PlanEditor.tsx` + `src/lib/plan-steps.mjs`）

- 原来计划是**只读**渲染（ActionCard 里的 `<Markdown>`）。现在条目可**勾选 / 改字 / 删 / 加**，
  改完点「交给 Codex」→ 序列化后作为**一条用户消息**发回。
- ⛔ **语义边界（它决定了整个实现，不是细节）**：计划的权威状态在**模型侧** ——
  `turn/plan/updated` 由引擎发、`update_plan` 是模型自己的工具，我们**传不进去**。
  所以语义是「用户改了计划 → 让模型按新的执行」，**不是**"直接改引擎里的计划"。
  编造"已同步到引擎"的假象比不做更糟（用户会以为改完必然生效）。
- **解析/序列化放 `src/lib/plan-steps.mjs`**（本项目惯例，同 `image-src.mjs` / `enhance-hints.mjs`）：
  纯函数零依赖 ⇒ 预检能**直接跑真断言**（`await import`）而不是 grep 源码文本 ——
  "查标识符存在"是假绿高发区。组件只管交互。
- 本地编辑按 `plan-edit-v1:<itemId>` 存 localStorage：改了还没交回就切会话/关窗口，回来还在；
  「交给 Codex」后清掉（内容已进对话，再留着会与模型后续产出的新计划打架）。
- 解析容错：第一条条目**之前**的普通行 → intro（原样展示、不编辑）；条目**之后**的普通行 →
  并入最后一条 —— **丢内容比格式难看严重得多**。预检【87】10 条。

### ② 新鲜上下文的 reviewer：写的人不审自己（`electron/builtin-agents.ts`）

- **做法**：随应用种入一个内置**子智能体**「评审（新鲜上下文）」（**幂等**：已存在就不动，
  尊重用户对它的编辑与停用），复用现有委派链路（`agent_invoke` → `runDelegatedTask`）。
- ⛔ **为什么用子智能体而不是新工具**：那条链路已经解决了起会话、安全闸（防套娃 / 独占锁 /
  身份闸）、结果回流、会话登记 —— 新工具会把这些重写一遍，还可能绕过闸门。
  而且子智能体在界面上**看得见、改得动、停得了**（不搞隐形魔法）。
...
⚠️ 这两项都动了 `developer_instructions`（新增一节）⇒ **config.toml 会因漂移检测被重写一次**，
属预期行为（不是 bug）。

## 09-23 「排队消息自动启动后，聊天区里看不到那条消息」（已修）

**症状**（用户反馈）：上一个回合结束汇总时，排队消息被自动发出 —— 聊天区**不出现那条用户消息**，
agent 只短暂显示「正在回复」然后就结束了。

**取证**（真实 app-server + mock 上游，独立 `CODEX_HOME`，不在用户 profile 上跑）：
- 排队消息是**引擎自己**启动成新回合的：`turn/completed` 之后 9ms 就发 `thread/queue/changed` +
  `turn/started`，队列随即清空 ⇒ 应用侧 `thread/queue/list` 拿到 `data: []`，**根本不参与建回合**；
- `turn/started` 的回合快照 `items=[]`、`turn/completed` 的快照 `items=[agentMessage]` ——
  **引擎从不在这两处带 userMessage**，用户消息只走 `item/started` / `item/completed`（实测两条都带完整正文）；
- ⇒ 「回合容器」只能由事件流自己建立。只要 userMessage 的 item 事件比 `turn/started` **早到**
  （同一 tick 内连投一串事件，顺序会抖），旧实现 `mergeItem` 对**未知回合**会静默丢弃该 item ⇒
  该回合的用户消息与流式内容一起消失，且再无补救机会（`turn/completed` 快照里没有它）。
  正常发送没事，是因为 send 路径先用 `turn/start` 响应 + `hydrateTurnUserMessage` 把回合落进本地
  （外面还叠一层乐观气泡）；排队自动启动 / 立即注入 / 自动续接都没有这层保护。

**修法**：`mergeItem`（`src/features/app-view/helpers/stream.ts`）在回合未知时，用**该 userMessage**
把回合建出来。判据下沉成纯函数 `src/lib/turn-item-merge.mjs`（守卫【111】真跑 + 接线顺序断言）。
只认 userMessage ⇒ 不会造出「没有用户消息的孤儿回复」（09-19 修过的那类）。正常发送路径不受影响
（回合已存在，判据直接返回 null ⇒ 逐位保持原行为）。

**验证**：把渲染层归约模块用 vite lib 模式打成 CJS，喂**真实事件序列**（正常时序 / `turn/started` 晚到 /
连 `item/completed` 都晚到）⇒ 修复前 2/3 场景丢消息，修复后 3/3 可见；`npm run check` 9/9 步通过。

## 09-23 两条正文之间的过程归成一个「带概要统计」的折叠块

**需求原话**：「在两条正文消息之间新增一个折叠效果：无论中间执行了多少工具和命令，最终都归并为一个可折叠
区块显示，展开前只显示概要统计（如运行了几条命令、编辑了几个文件等）。统计需按工具的实际调用内容进行
映射归类，而非仅按工具名计数。现有的『相同工具连续调用超过三次时触发折叠』逻辑需与此保持一致，避免重复
折叠或冲突，请在实现时兼顾这两种折叠规则。」

**落点**：完成态折叠块（`TurnFoldStream` 的两条分支）本就把「两条正文之间的全部过程」收成一个组 ——
本次只补两件事：① 折叠头右侧显示**按调用内容归类**的概要统计；② 关掉内层那层截断，避免对同一批内容折两次。

- **统计纯函数** = `src/lib/tool-call-classify.mjs`（守卫【112】真跑 32 条）。⛔ 核心判据是
  **按调用内容而不是按工具名**：引擎把绝大多数动作都报成同一个 `commandExecution`，实测某回合 37 次调用里
  24 次 `Get-ChildItem`/`Get-Content`/`git status`（查看）、9 次 `Select-String`（搜索）、只有 3 次真构建。
  按工具名只能数出「运行 37 条命令」，按内容才数得出「查看 18 次、搜索 9 次、写入文件 6 次」。
- **单位不许混**（否则数字是错的）：`编辑 N 个文件` 的单位是**文件数**（只来自 `fileChange.changes[].path` 去重）；
  命令里直接写文件单独进 `写入文件 N 次`（一条命令可能写多个文件，数不清）。其余桶都是**次数**。
- **两条折叠规则各管一段、不重叠**（用户明确要求「避免重复折叠或冲突」）：
  · **完成态**：外层折叠块独占「两条正文之间」的收敛 ⇒ 内层 `CappedToolSequence cap={false}` 关掉
    「同一工具 >3 条」那层。否则同一个折叠块里会出现「已收起 N 条」（折两次），而且停止回合
    （`keepProcessOpen`）外层默认展开时内层仍收着 ⇒「点开也看不到做到哪了」（09-18 修过的老问题）复活。
  · **运行态**：没有外层折叠块 ⇒ 那层 >3 截断照旧生效，是那一阶段唯一的收敛机制。
  ⛔ **切分改名的教训**：`CappedToolSequence` 的开关**必须叫 `cap`** —— 曾想改名 `capped`，与
  `CappedToolRun` 的 `Capped` 前缀撞名，容易看成"同一个东西"。`cap` 短、无歧义，且有默认值 `true`（既有调用点零改动）。
- **展开前只显示概要**：非首段折叠块的标题直接用统计文案（没有可统计的步骤时回退到原意图摘要）；首段保持
  「耗时 X · 场景」这类回合收尾标题，统计走右侧独立槽位（`.wb-fold-stats`），**不与标题互换**。
  ⛔ 不要把统计并进标题串 —— 09-20 用户明令删掉过「N 项失败」那种拼接文案。

**验证**（真机，隔离 profile，不碰用户 userData）：
- 离线：分类器 19 条样例逐条断言（含两个实测踩到的假阳性，见下）；真实 rollout 复核 —— 截图那个回合得到
  「查看 18 次、搜索 9 次、写入文件 6 次 等 5 类」。
- 真机 **8/8**（遍历侧栏挑出工具最多的会话，74 张工具卡）：折叠头显示「调用服务 54 次、搜索 3 次、查看 2 次 等 4 类」；
  展开后 **54 张工具卡都在**（不是空壳）；外层折叠块内**没有**嵌套的「已收起 N 条」；完成态默认收起。
- `npm run check` 全绿（预检 0 ✗，含新守卫【112】32 条）。

**⛔ 命令意图识别的两个实测假阳性（都进了守卫，别改回去）**：
① **管道格式化段会污染判定**：`npm run build:electron 2>&1 | Select-Object -Last 15` 尾巴上的
`Select-Object` 只是限行输出，一起参与判定会被算成「查看」⇒ 先剥掉**非首位**的纯展示型管道段
（`Select-Object`/`Where-Object`/`Format-*`/`head`/`tail`…；但 `Out-File`/`Set-Content`/`Tee-Object` 一律保留，
它们是写文件信号）。
② **`=>` 会被重定向正则误吃**：`l => !re.test(l)` 里 `>` 后跟 `.test` 形状上像 `> x.ext`，第一版把一堆
JS 单行脚本误判成"写文件"⇒ 重定向的 `>` 必须**前面是空白/分隔符**（`2>&1` 与 `=>` 因此都不命中）。
③ 复合命令里的**有后果动作**不能被同一条里的只读子命令冲淡：`git add …; git commit …; git log …` 归「运行命令」
（否则 `git log` 会把整条判成"查看"）。

**验收脚本的取样坑（本轮踩到，通用）**：折叠组**收起时 `innerText` 是空串**，用它判"标题/统计有没有渲染"会得到
假红（本轮据此一度以为出现了空标题折叠组，按 `textContent` 复核后为 0 个）。判「内容是否存在」一律用
`textContent`（与 AGENTS 开头「判据不能用会随渲染状态变化的量」同源）。

## 09-23 运行中显示增强按钮 + 消息区底部留白归零

**① 增强按钮不按「回合是否在跑」隐藏**（用户报「运行过程中，输入框已输入内容时增强图标不显示」）：

- 原先两处都带 `&& !activeThreadRunning`（按钮 JSX 与 `enhanceAnchorVisible`），运行中把按钮**整块不渲染**。
  该门槛已**永久移除**：运行中输入框里写的是**下一条消息的草稿**，与在跑的回合无关；
  长任务里打草稿的用户不该看到按钮凭空消失。发送按钮在同一处已支持「运行中有草稿 ⇒ 排队发送」，本就承认这种输入。
- **连带效果（知情）**：`enhanceAnchorVisible` 必须与按钮条件逐字一致（否则出现"按钮没画出来却去弹气泡"），
  所以**运行中打草稿现在也会弹提示气泡**。频控口径未动（发送后 / 长输入 ≥120 字 + 3 分钟冷却）。
- 守卫【32】改为两条一起卡：① 两侧条件必须是**同一表达式**；② 旧门槛一个字都不许回来。

**② 消息区底部留白归零**（用户要求「内容超一屏时，最新一条消息紧贴输入框正上方」）：

- 真机实测空隙 **60px** = 最后回合 `margin-bottom: 32px` + `.timeline` `padding-bottom: 28px`。
- 改动两处：`.timeline` 的 `padding-bottom` → **0**；
  `.timeline > .turn-group:not(:has(~ .turn-group)):not(:has(~ #chat-anchor))` 的 `margin-bottom` → **0**
  （只归零"最后那个回合"；回合之间的呼吸空间不变；乐观气泡在场时保持原 margin）。
- ⛔ **与钉顶的耦合**：`contentBottomOf = scrollHeight − 尾部留白`，而**回合 margin 不被扣除** ⇒
  底部只要还留着可见空白，"贴底跟随"就会停在空白上方。所以必须 padding **和** 末回合 margin 一起归零。
- **A/B 对照（同一个已热身 profile、同一条 `--only send-anchor` 命令）**：
  改动前 **9/15**、改动后 **12/15**；改后转绿的 3 项正是「最新正文始终可见 / 视口不回跳 / 追赶」——
  **不是回归，是顺带修好了既有失败**。剩下 3 项（第 1 条内容不满一屏 `max=0`、`atBottom` 在那儿本就无意义）
  在改动前**同样失败**，属既有状态。
- 实测残留约 **10px**（未定性来源）；真机截图上最新内容已紧贴输入框。运行态的尾部呼吸由
  `.timeline-bottom-spacer.compact` 负责，未动。

## 09-23 折叠/展开后自动把内容底部贴回视口底

**用户报**：「思考会把底部撑出来这么空白，他折叠之后，这个没有自动消失」。

**根因**（真机实测 + 打点，不是推理）：非钉顶路径下 `update()` **只处理「内容变高就往下补」**
（`dist > 0` 才动），内容高度变化后的 `dist` 是个**谁也不会去消的残留值** ——
完成态滚到底、折叠一张视口内的思考卡：`scrollHeight 3188 / scrollTop 2197 / max 2589`
⇒ 可视空白 **280px**。更糟的是紧接着的 `dist > clientHeight * 0.25 ⇒ stickToBottom = false`
会因这个变大的 `dist` 把贴底跟随**关掉** ⇒ 此后再无自动恢复，手动滚一下才回来（正是用户描述的现象）。

原本指望 `.turn-group` 上的 `turnResizeObserver` 兜住，**实测靠不住**：视口外的回合被
`content-visibility: auto` 跳过（内部变化根本不做布局 ⇒ 不触发 RO），观察名单也可能没覆盖到新出现的回合组。

**修法**（`part04/02-seg/01-hook-events-bottom-state.tsx`）：
- `update()` 新增分支：**非钉顶** + 「内容底部 `cb` 相对上次变化 > 1px」+「用户正跟着底部（`stick`）」
  ⇒ 把内容底部重新贴回视口底（`contentTailTarget`）。只认「高度变了」这一个事实，
  **不引入任何"用户意图"启发式**（09-12 已证那套不可靠）。
- 加 `transitionend`（**只认 `grid-template-rows`**，其它过渡不理会）做兜底：折叠高度是 grid 过渡，
  过渡结束必重算一次，不依赖 RO 是否触发。
- 记账用 `finally`（`lastBottomRef`）：`update()` 有多处提前 return，漏记一处下次判据就失真。

**验证**（真机、隔离 profile、只读不写会话）：
- 单会话逐帧：展开 **3522px** → 折叠后 **28px**，`dist` 全程 **0**（修复前该场景 280px+）。
- 多会话批量（逐张展开→折叠，6 个会话）：所有折叠点 `dist ≤ 4`。
- ⛔ **取样有效性必须做成断言**（本轮踩到假绿）：第一版只挑「collapsed + 与视口相交」，
  结果挑中一张 **body 为空**的卡（展开 28px、折叠还是 28px）⇒「折叠让内容变矮」这个场景
  **压根没发生**，5/5 全是假绿。现在先试展开、**增高 > 100px 才算有效样本**，否则换下一张。
- ⚠️ 已如实记下：`.turn-group` 的 RO 在这一刻**一次都没触发**（打点 0 次），
  `dist` 的收敛同时也是浏览器在 `max` 变小时对 `scrollTop` 的钳制结果
  —— 两者对用户是同一结果，但**不能据此宣称"只靠新分支就够"**。

### 补丁：钉顶期间折叠思考卡也会留缝（09-23 用户追报）

用户原话：「思考板块折叠后，缝隙也跟着效果」。

**确定的结构缺口**：上面那条修复是 `else if`，而**钉顶（pin）期间它根本不会执行**
（`if (pinMine) {...}` 命中后 `else if` 被跳过）。而钉顶那条跟随**只往下补**
（`scrollTop + dist`，步长 48px 是为打字机逐字重排的抖动设计的），**永远不会把视口拉上来**
⇒ 回合运行中（用户刚发过消息、钉顶还在）折叠思考卡后，内容底部上方留一条缝，且再也不会自己消掉。

**修法**：`pinMine` 分支内新增「内容变矮（`shrankBy > 1`）且内容确实超出一屏 ⇒ 立刻贴合」，
增长方向仍走原来的 48px 步长（一个字不改）。
⛔ 只在 `cb − clientHeight > 0` 时接手 —— 内容不足一屏时锚点该待在 54px，那是钉顶的职责。

## 09-23 四个元技能内置 + 技能安装门禁（提交 `961827b` / `b520164`）

**用户要求**：「技能配置与集成…内置到 Codex 并相互协同」+「安装技能审查技能，并设置强制规则」。
**核实后的实情（重要，别重复劳动）**：三类里**两类的后端早已落地**——
`electron/memory-hygiene.ts`（八层清理规则表 + 白名单动作 + 二次确认 + 守卫【107】）、
`electron/memory-lessons.ts`（四分类 + 优先级）、内置技能 `memory-distill`（长期记忆）。
所以本轮**只补缺口，没有重建任何已有能力**。

### 新增的四个内置技能（`electron/builtin-skills/09..12`）

| 技能 | 触发时机 | 与现有板块的适配 |
|---|---|---|
| `self-review` | 多步任务收尾 / 被用户纠正 / 走了弯路又绕回来 / 用户说「复盘一下」 | 四问 → 结论按分类落 `lessons/<类>.md`；**同现象第二次 ⇒ 升级成技能**（读 `skill-authoring`） |
| `memory-classify` | **写任何一条记忆之前** | 一条一现象；优先级 纠错 > 任务经验 > 工作流SOP > 用户偏好；先读目标文件去重 |
| `memory-hygiene` | 看到「记忆水位 ≥90% 蒸馏线」/ 用户说「记忆太满了」 | 八层规则表（能归档就不删 / 保护项永不自动清 / 留痕）。**引擎不直接执行清理**：三个动作走 IPC + `confirm!==true` 拒绝 |
| `skill-audit` | **装任何技能之前**（用户要求 / 自己搜到 / 导入 SKILL.md） | 五查：结构 / 危险模式 / 权限面 / 来源与供应链 / 提示注入；未通过不得安装 |

### 集成点（四处同源，技能名一律走 `skill-pack.ts` 的常量）

1. `builtin-skills.ts` 的 `entries` —— 启动时写进 `$CODEX_HOME/skills/<名>/SKILL.md`（尊重用户的停用态）；
2. `skill-discipline.ts` 的 AGENTS.md 守则：**第 3 条由「缺技能就自主安装」改为「先审查、再安装」**，
   并新增第 9 条（收尾复盘）、第 10 条（记忆分类与整洁）；
3. `developer-instructions.ts` 新增第 10 条 `GATE_AND_REVIEW_INSTRUCTIONS`（每轮下发：强制门禁 + 收尾复盘 + 记忆规则）；
4. **`skills:import` 导入口**（真缺口修复）：此前只有市场安装走 `auditSkill`，
   本地导入 SKILL.md **完全没过审查** ⇒ 绕门禁的后门。现导出 `auditSkill` 并在导入口调用。

### 验证方法

```bash
node scripts/check-preflight.mjs     # 看 【113】（四个技能真跑落盘 + 结构完整）
                                     # 与 【114】（四处同源 + 导入口必须审查）
node scripts/check-bag-types.mjs     # 【93】Bag 防漂移
```
⛔ **内置技能正文里不要写危险命令的字面量**：语料会被预检【86】扫描，我第一版写了 `del` 递归强删等
示例，直接命中 `wipe-windows` ⇒ 改成按「形态」描述（「一次删掉整个盘符的删除命令」）。
⛔ **`check` 是一条链，不能只挑几步跑**：上两轮我只跑 tsc + vite + 预检，漏了 `check-bag-types`，
带着【93】的红提交了两次（`lastBottomRef` 没登记进 Bag）—— 见 `b520164` 的修复。
...
**用户口径（经一次更正，必须按最终版理解）**：
1. 先提的要求：「当用户手动停止程序/任务的运行状态时，排队中的消息不应被自动发送，而应保留并停留在
   输入框中，等待用户后续自行处理」；
2. ⛔ **随后的更正**：「不是停留到输入框里面，是停留在输入框**上面**；要是排队好几个消息怎么办，
   要保持排队消息、停留在输入框上面」。
   ⇒ 第一版把排队消息**从引擎队列摘走、拼进输入框草稿**是**错的**（多条会糊成一条草稿），已整体删除。
   正确语义 = **排队消息原地不动**（继续挂在输入框上方那张排队卡里，徽章仍是「排队中」），
   只是**不自动发送**；用户自己决定「立即 / 编辑 / 删除」。

### 唯一拦截点：`turn/completed` 的「自动启动下一条排队消息」

`interrupt()`（`part08/03-seg.tsx`，三个手动停止入口 —— composer 停止键 / `/stop` / 计划模式中断键 ——
全都经过它）**同步**写一条记录 `bag.manualStopRef`（Map：threadId → 被停止的 turnId）：

- ⛔ 必须是 **ref** 且写在**第一个 await 之前**：`turn/interrupt` 的回包与引擎的回合结束事件是两条
  独立异步流、事件可能**先到**（写在 await 之后必漏判）。守卫【115】卡这条顺序。
- ⛔ **不动排队消息**：不 `thread/queue/delete`、不写回输入框（守卫【115】的负面断言盯着这条）。
- 按 **turnId 精确比对**：只对这一次生效；之后别的回合正常结束照旧自动启动。用完即删。

`07-turn-completed-settle.tsx` 里：`wasManualStop` 为真 ⇒ **短路**「自动启动排队消息」，
并顺带短路「截断空转的自动续接」（`isTruncatedEmptyTurn(...) && !wasManualStop`）——
手动停掉的长思考回合在**形态上**与截断空转一模一样（思考很长 + 正文为空 + 无工具调用），
不拦就会在停止后 2.5s **自动再发一条「接着写」并启动新回合**，同样属于用户禁止的「自动发送」。
⛔ 短路只加在 `if (wasManualStop)` 这一支，`else` 分支的自动启动**一个字没动**（守卫【115】卡住两支）。

### 为什么不需要动引擎队列（真机实测，别退回旧方案）

- 被**中断**的回合结束时，引擎**不会**自己释放队列：真机 A/B（一次性隔离 profile + 真回合）实测
  停止后 2.5s / 12.5s，排队卡里那条**仍在**（`.queued-message` = 1、徽章仍「排队中」）、
  输入框为空、`turn-group` 数不变、`userTexts` 里**没有**它 ⇒ 不自动发送。
- 对照（摘掉修复）：停止后 2.5s 它**已变成用户消息**、`turn-group` 1→2、仍在跑；打点
  `queue-arm{why:"auto-start"}` 指认是**宿主** `turn/completed` 那条自动启动发的 ⇒ 拦这里就够了。
- ⚠️ 已知边界：语音插话 / 手机端停止走**主进程**的 `turn/interrupt`，不经过 `interrupt()` ⇒ 不享受本语义。
- ⚠️ 语义说明：这些消息**仍是排队消息** ⇒ 用户之后**自己发起**的下一个回合结束时，它们会照常被发出
  （这是「排队」的固有含义，卡片徽章也这么写）。要一条都不发就点「删除」；要马上发就点「立即」。

---

## 09-23 WorkBuddy 日志对照后的 P0 四项（构建指纹 / Ctrl+Enter / 听写不吞字 / 每轮当前时间）

来源：`docs/WORKBUDDY-CHANGELOG-REVIEW-2026-09-23.md`（那份对照分析的 P0 清单）。守卫【116】22 条。

1. **构建指纹** —— 回答「你正在跑的到底是哪一份产物」：
   `vite.config.ts` 的 `define` 在**构建期**注入 `__BUILD_STAMP__`；`buildStamp` 改为取自它
   （原值是硬编码 `"20260831-1830"`，过期 23 天无人发现 = 一直在给用户假信息）；
   `bundleFile` 取 `import.meta.url` 的**正在执行**的 bundle 名（含内容 hash）。
   侧栏账号菜单「检查更新」页显示这两行 + 「复制诊断信息」按钮（版本 / 构建 / 产物 / 引擎信息 / 平台）。
   ⛔ 别改成"运行时去读 `dist/`"：那只能说明磁盘上有什么，不是引擎实际加载了什么 —— 而 09-23 那次
   事故（用户看的实例不是最新构建、我无法自证）要回答的正是后者。
2. **回车语义**（`onComposerKeyDown`）：`Enter` 发送；`Shift / Ctrl / Cmd + Enter` 换行。
   ⛔ **先挡输入法组字**（`nativeEvent.isComposing || keyCode === 229`）：打中文时按回车确认候选词
   会把半截消息直接发出去 —— 这是之前一直缺的守卫。
   ⛔ Ctrl/Cmd+Enter 的换行**必须自己插**：`document.execCommand("insertLineBreak")` 对
   `contenteditable="plaintext-only"` 是**空操作**（真机实测：按下去输入框毫无变化，比"误发送"更难查）
   ⇒ 用项目既有约定 `execCommand("insertText", false, "\n")`（见 `src/features/composer/Composer.tsx` 粘贴分支）。
3. **听写不打断手动编辑**：回填逻辑抽成纯函数 `src/lib/dictation-merge.mjs`（预检**真跑**断言）。
   听写回调给的是**整段字幕**（不是增量）⇒ 只能替换「上次写进去的那段」；用户期间动过输入框
   （用 `dictationWrittenRef` 位标比对）就把他的内容**并入基准**、字幕接在后面，**永不覆盖用户输入**。
   ⛔ 位标在「点麦克风起听」与「听写结束」两处都要清零，否则第二条听写会被误判成"用户动过"。
4. **每轮注入当前时间**：`src/lib/now-block.mjs` 生成 `[当前时间]…[时间结束]` 隐藏块，由 send 路径
   （**发送与排队两处**）附着在**当轮消息**末尾；`harness-block-strip.mjs` 负责在气泡/标题里剥掉
   （模型看得见、用户看不见；用户在正文里单独提到该标签不会被误剥）。
   ⛔ **绝不能写进 `developer_instructions`**（config.toml）：那份文本要落盘并被 `instructionsOutdated`
   比对，放一个每轮变化的值会让"指令过期"**恒真** ⇒ 反复重写配置；而且时间会**冻在写盘那一刻**，
   比不注入更糟（模型会当真）。
...
用户附了两张截图：图一（我们）正文之间一条条铺着 `已运行 "D:\…\pwsh.exe" -Command "…"` 原始命令，
「看着很变扭」；图二（对标）正文之间是**一行行意图词芯片**（`运行命令` / `修改、运行校验：<路径>`）。

**⚠️ 过程：先做错一版，再改对 —— 正确解法 = 双层折叠（两个需求并存，不是二选一）**。
第一版按确认过的口径做了"正文锚点化"（`planCompletedFold` 改成「有正文就是正文锚点」= 正文之间各自
成块），**当天被否**——用户原话：「你先加了一个正文中间折叠，运行过程不折叠吗」。原因：过渡正文一旦
留在外面，一整轮的过程就被切成一堆小折叠块、过渡正文全部裸露 ⇒ **旧消息再没有"整段运行过程折叠" +
汇总消息**，时间线瞬间膨胀。
- **外层** = 一整轮的过程（`FoldGroup`，回合收尾皮肤「耗时 X · 统计」），**默认收起** ⇒ 旧消息依旧只有一行；
- **内层** = 外层展开后，夹在正文之间的每段过程各自再收成一个意图词芯片（`NestedProcessRuns`）。
  ⛔ 两层是**不同粒度**，不是"对同一批内容折两次"：外层管"整轮收不收"，内层管"这段过程说不说"。
  真机实测（会话 5 turn#1）：收起态一行「耗时 9m 38s · 查看 10 次…」、散落 0；展开后 11 枚芯片 +
  10 段过渡正文，芯片默认也收起。

**保留下来的三处改动**（守卫【118】21 条，真跑纯函数断言；真机 10 条历史会话逐回合量过）：
2. **单条过程也收**（`turn-fold.ts` `buildSegments`）：flush 不再按条数分叉（原来只有 ≥2 条才折，
   1 条直接内联）。这个洞在没有最终答复的回合（segments 分支）一直存在，与结构回滚无关。
3. **命令行剥壳**（新模块 `src/lib/command-display.mjs`，纯函数供预检真跑）：
   - `stripShellLauncher`：剥掉宿主包上去的 shell 启动器（`"…\pwsh.exe" -Command "…"`、`-lc`、`/c`），
     认不出来**原样返回**（宁可显示带壳原文，也不许把用户命令改错）；
   - `commandTarget`：取命令里的**文件目标**（路径归一正斜杠、超长**从左截**保文件名），
     取不到返回空 ⇒ 摘要退回「运行命令」；
   - `displayCommand`：命令卡片表头用的展示形态（剥壳 + 压空白 + 保头截断）。
   接线：`foldAtomOf` 的 commandExecution 目标走 `commandTarget`（**不许**再拿原始命令行当 topic）；
   `CommandExecutionCard` 表头用 `displayCommand` —— ⛔ **展开里的完整命令仍是引擎原文**
   （用户要照着复现），剥壳只用于展示。

**摘要文案**：非首段折叠块标题 = `computeFoldSummary`（意图词，如「运行命令」「修改文件、运行命令：
…/01-seg.tsx」）；概要统计**移到右侧小字**（`stats=` 两条完成态分支都保留——09-23 用户要过的
「展开前能看到运行了几条命令」没删）。首段仍是回合收尾皮肤（耗时/停止原因，守卫【50】的契约）。
⛔ 两条完成态渲染分支（planCompletedFold / 通用 segments）的 title 与 stats **必须同时改**——
同款陷阱【50】【112】⑤都踩过。

**实测踩坑（都在守卫里钉住）**：
- ⛔ URL 不是文件目标：`https:` 里的 `s:` 一度被盘符正则吃掉，摘要成了「运行 s://api.pptoken.org」。
  盘符只认 `X:` **且后面紧跟分隔符**（写成 `X:\` 会把分隔符吃掉，带空格的路径
  `D:\Codex Harness Desktop-refactor\…` 就匹配不到了——两个方向都踩过）。
- ⛔ 两个目标拼接超上限时**只留第一个**，不要截断第二个（尾巴切成半截比不展示更糟）。
- ⛔ 真机扫描时别把别的 `.wb-fold` 当折叠块：思考卡内部、命令输出区也用这个类；
  判据用「有没有 `.wb-fold-header`」（不带头的 62 个、带头的才 24 个）。
- ⛔ 真机点击会话要点 `.thread-row` 里的 **button**（行容器中心可能落在 small/操作区）。
- ⛔ 探针的 `finally` 里 `process.exit` 会吞掉 try 块的异常栈 ⇒ 必须先 `catch` 打印再进 finally。

---

## 09-23 晚：钉顶回归（我下午的折叠提交触发的竞态）+ 运行态过程自动折叠

**① 钉顶回归（用户：「钉顶效果又没了」）—— send-anchor 一度 6/10、gap=400+：**
根因在 `02-pin-scroll-anchor.tsx` 的 `key = isNewTurn ? turn-… : "opt"`：新回合组已建、
真实用户消息**还没渲染**的那一帧，isNewTurn 已真而 `.user-message` 查不到 ⇒ 锚点落到
乐观气泡（打点 aCls://），key 却被领成 turn-…；真实消息渲染出来时 key 没变 ⇒ `first` 恒假 ⇒
不走 first 分支 ⇒ `anchorElRef` 停在随即被卸载的气泡上 ⇒ 气泡一卸载钉顶被释放。
**这个竞态一直存在**，下午的折叠提交只是推移了时序把它暴露出来（b520164/1ad0453 同路径恰好
走了 first 分支）。修法：key 描述「锚点元素是谁」—— 锚点是气泡就老实用 "opt"，真实消息渲染
那刻 key 变化 ⇒ 必然走 first ⇒ 交接完成。修后 send-anchor **15/15**（gap=36）。
⛔ 另修一条**过时验收判据**：「视口没被贴底接管」原来用 scrollTop<max 当证据 —— 09-15 起
pad 改「只补缺口」，设计上滚到底就正好是落点（scrollTop==max 是预期），短内容下旧判据恒假。
改成「没有出现贴底 + 消息远离顶部同时成立」。

**② 运行态过程自动折叠（用户：「正文中间工具调用没有在下一个正文出来后自动折叠」）：**
运行分支原来全部内联（只靠「同一工具 >3 条」收敛）。现在用 buildSegments(units,false) 的
shouldFold（= 已被后续正文隔开）把那一段收成 FoldGroup（autoFold 播收起动画，形态=完成态
内层芯片）；⛔ 还在长、还在跑的末段保持内联 —— 不能合进摘要（运行态老规矩）。
守卫【112】⑤ cap={false} 计数 2→4（两条完成态分支+内层两段+运行态折叠段），新增两条：
运行态折叠段 autoFold + 末段仍内联；【118】意图摘要 4 处。

**⛔⛔ 本轮最大教训（流程级）：做 A/B 二分时用 `git checkout <commit> -- src electron scripts`
切换、被用户打断在恢复之前 ⇒ 工作树+索引停在被测版本上，我接着在上面编辑，差点把用户刚验收
过的双层折叠（NestedProcessRuns）弄丢。恢复时还撞上 .git/index.lock（被杀任务残留）。
规则：A/B 切换必须「切回 → 立即验证 git status 干净 + 关键符号还在」再继续；
被中断的 A/B 恢复后第一件事就是核对工作树。**
另：git checkout 往返会把文件行尾写成 CRLF，与编辑工具写入的 LF 混合 ⇒ 守卫里
\n  }\n 这类定位全部失匹配（【112】运行态断言假红）。守卫读文件后统一 .replace(/\r/g,"")。

- **系统托盘（09-23 用户：「要在系统托盘里面常驻，系统托盘右键功能菜单齐全一下」）**：新增 `electron/tray.ts`
  （主进程）；图标由 `scripts/build-tray-icon.cjs` 生成三份 —— `build/tray.png`（32×32 彩色，Win/Linux）、
  `build/trayTemplate.png`（16×16 纯黑+alpha）+ `@2x`（mac 模板图：系统只读 alpha 并按菜单栏明暗反色，
  文件名必须以 `Template` 结尾）。
  ⛔ **`build/**` 不在 electron-builder 的 `files` 白名单里** ⇒ 三个托盘图已显式加进 `package.json.files`，
  否则打包版托盘是个"看不见的图标"（不报错、最难查）；运行期仍做 `existsSync` + `image.isEmpty()` 双查 + 打日志。
  **菜单**（右键；mac 是左键，符合平台约定）：显示/隐藏主窗口 · 实时状态行（`运行中：N 个任务`／`空闲`）·
  `关闭窗口时最小化到托盘`（勾选框）· 打开数据目录 · 查看引擎日志 · 退出。
  ⛔ **菜单在弹出时现建**（`popUpContextMenu(Menu.buildFromTemplate(buildAppTrayMenu(deps)))`），**不用
  `setContextMenu` 缓存** —— 状态行与"显示/隐藏"是实时值，缓存住就会说假话。
  ⛔ **菜单里没放「新建任务」「跳设置页」**：那需要一条主→渲染命令通道（preload + vite-env.d.ts + 渲染层监听
  三件套），属另一个改动，不顺手加。
  **「关闭窗口时最小化到托盘」默认关**（新增 `app-settings.closeToTray`，入口只在托盘菜单）：开着时
  `BrowserWindow` 的 `close` 变成 `hide`（引擎/调度器/渠道机器人/在跑回合都不中断），退出走托盘「退出」或 Cmd+Q。
  ⛔ 三条硬约束：① 该分支必须排在"还有 N 个任务在跑"确认框**之前**（隐藏不中断回合，弹警告只会吓人）；
  ② **任何退出路径都要先把 `mutableState.closeConfirmed` 置真**（托盘「退出」+ `before-quit` 两处），
  否则 `close` 守卫会把 quit 变成 hide ⇒ **点了退出却只是把窗口藏起来**；③ `before-quit` 里
  `destroyAppTray()`（Windows 上不销毁的托盘图标会残留到鼠标划过才消失）。
  `second-instance` / mac `activate` 统一走 `showMainWindow()`（不存在则建、最小化则还原、隐藏则显示再聚焦）——
  ⛔ 别在三处各写一份 `mainWindow?.show()`：窗口销毁后它只是 no-op，用户点了"显示主窗口"会什么都没发生。
  验收：预检【123】21 条（图标真 PNG + 尺寸 + 随包白名单 + 平台分叉 + 菜单现建 + 条目面 + 退出链路顺序 +
  closeToTray 分支位置）。运行时实测（隔离 profile，`app-settings.json` 预置 `closeToTray=true`）：
  ① `[tray] 系统托盘已创建（图标 …build\tray.png；closeToTray=true）` —— 托盘真建起来，且开关读到的是
  预置值（排除"app-settings 缓存读错目录"那一类）；② 触发关窗（CDP `Browser.close`）→ 主进程打出
  `[tray] 关闭被拦截 → 隐藏主窗口`、进程仍存活 ⇒ **隐藏分支端到端成立**；③ 同触发在"置真"生效时
  干净退出 ⇒ 退出链路没被开关卡住；④ **A/B 反证**（临时去掉 before-quit 的置真）→ 同一触发变成
  "隐藏且退不掉" ⇒ 证明那行置真是必需的而非防御性冗余。
  ⛔ 触发关窗**只能用 `Browser.close`（或原生 WM_CLOSE）**：它走 BrowserWindow 的 `close` 事件；
  **`window.close()` 不走**（直接销毁窗口 → `window-all-closed` → quit），拿它验证会得到假红（本日实测：
  它把进程直接退掉了，掩盖了真实行为）。

- **归档提示浮层（09-23 用户两句话定稿：先「放对话框正中间」→ 更正「**对话框上方正中间**啊，不是应用正中间」）**：
  「已归档 + 会话名 + 查看归档 + ✕」那颗浮层改成 **贴在输入框上方、水平居中**、**3 秒**后自动消失（原 5 秒）。
  · **渲染点从 timeline 挪进 composer**（`02-main-stage/03-composer.tsx` 的 `.composer-wrap` 内）——
    留在 timeline 会跟着消息区滚走；且两处各渲染一份会一上一下同时出现，预检【124】盯这一点。
  · CSS（`.archive-toast`）：`position: absolute; left: 50%; bottom: calc(100% + 10px); transform: translateX(-50%)`。
    **绝对定位 ⇒ 不参与布局**（输入框不会被推上去、3 秒后又落回来）；包含块是 `.composer-wrap`
    （它本身 `position: relative`，【124】也钉这条 —— 谁把 relative 去掉，浮层就跑到 workspace 去了）。
    ⛔ **入场动画的 `transform` 会整体覆盖静态位移**（不是叠加）⇒ `@keyframes archive-toast-in` 里必须
    自带 `translateX(-50%)`，漏了动画一开始浮层就从居中位置跳到左边。
  · 实测（真实渲染 + 同结构节点插入 `.composer-wrap`）：水平中心相对输入框偏移 **0px**；
    浮层下沿距输入框上沿 5~10px（5px 是入场动画中途量到的，动画收敛后 10px）。
  ⛔ 本沙箱里引擎起不来（连 `.status-dot` 都没有）⇒ **真实归档触发不了浮层**，「3 秒消失」这条是按
  组件默认值 + 守卫断言的，没有跑到端到端。

- **被委派会话的记忆（09-23 用户：「子智能体跟专家团都有没有记忆板块跟功能，没有就完善好」）**：
  新增 `electron/delegate-memory.ts` + **三处接线**（`features/delegation.ts` 的 `runDelegatedTask`；
  `features/teams-agents-ipc.ts` 的 `teams:member-session` 与 `teams:invoke-member`）。
  **缺口**：主会话的常驻记忆 + 按需召回是在**渲染层发送路径**拼的（`send.tsx` 的 `readMemoryContext`
  + `recallMemory`），而委派回合由**主进程直接 `turn/start`** ⇒ 子智能体 / 专家 / 专家团主理人 / 成员
  此前**读不到任何记忆**（不知道用户偏好、项目背景、踩过的坑与纪律）。第四条成员入口
  （`teams:member-session` 带 `defer`）走渲染层 `expertRole` 分支，本来就有记忆，未动。
  **口径（与主会话逐字同源，别改）**：常驻层 = `memoryLayers.context(workspace, includeWorkspace)`
  （自带 `[Harness 常驻记忆 …]` / `[常驻记忆结束]` 标记）；召回层 = `memoryStore.recall(query, {workspace})`
  包成 `[Harness 相关记忆，仅供参考]` … `[记忆结束]` —— 显示侧（`user-refs` / `thread-backup` /
  `rollout-worker-source`）按这些标记**整段剥离**，换标记会把机器块漏进用户气泡 / 侧栏标题 / 导出。
  **门禁不新增开关**：沿用 `workspaceMemoryEnabled(workspace)`；关掉时与主会话 `context(ws,false)` 同口径
  —— 仍注入 L0 用户档案，只是不含项目层、不做召回。
  ⛔ **三条硬约束**（守卫【125】14 条钉死）：① 标记必须与主会话逐字一致；② **任何一步失败都降级为空串**
  （委派本身比记忆重要，抛出去 = 整个调度失败；两层**各自独立降级**，常驻层炸了召回层照跑）；
  ③ **工作区不许退到 `process.cwd()`** —— 那是应用安装目录，而 `workspaceMemoryEnabled()` 对它还会
  默认放行 ⇒ 静默读到一份不存在的工作区记忆（最难查的那种）。解析顺序：显式 `cwd` → 发起方会话的 cwd
  （`threadCwd`）→ 空（只给 L0）。
  **设置页「记忆」板块**新增一节说明（`MemoryCenterSection`：纯文案、复用既有 class、无新增 IPC/状态）。
  ⛔ 守卫【125】里 `process.cwd()` 那条**必须先剥注释再匹配** —— 模块说明注释里就写着 `process.cwd()`
  （解释为什么不用它），直接 match = 自造假红（与【112】同一个坑，09-23 二次踩到）。
  行为验证：stub 宿主加载 `dist-electron/delegate-memory.js`（不起 Electron），**22 条断言全绿**
  （解析优先级 / 两层独立降级 / 标记字面量 / 空 query 不召回）。预检 19 ✗ = 已知环境基线（node
  type-stripping / python tomllib / AUTOMATION_ZIP），与本次改动无关。
  ⚠️ 顺带查出（**未修，待确认**）：渲染层 `thread/memoryMode/set`（`01-ui-ready-mood-refs.tsx:138`）
  在 electron 侧**没有任何 handler**，被 `.catch(() => undefined)` 吞掉 ⇒ 疑似静默失效的旧通道。
  ⚠️ 另：`part02/01-mcp-teams-plan*` 三个文件头注释写的「**角色记忆**」在代码里零实现 —— 又一处
  「注释声称能力」（做能力盘点时以代码为准）。

- **IPC 桥生成器（09-23 用户拍板解决体检报告「最大瓶颈」：加一个 IPC 接口要手写多处、纯靠人肉同步）**：
  新增 `scripts/gen-ipc-bridge.mjs`（extract / generate / check / verify 四模式）+ `scripts/lib/gen-ipc-core.mjs`
  （纯生成函数，CLI 与守卫【2】**共用同源**，防两套实现漂移）+ `electron/ipc-channels.manifest.json`
  （**单一真相源**：一个方法一条 name / channel / paramsImpl / paramsDecl / returns / cast / note）。
  生成物：**内联在 `electron/preload.ts` 的 `gen:begin…gen:end` 标记区**（251 个 invoke 方法）
  + `src/vite-env.d.ts` 的 `codex: {` 块内 `gen:begin…gen:end` 标记段。
  ⛔⛔ **生成内容绝不能放独立文件再 import**（曾做成 preload.generated.ts）：沙箱化 preload **不许
  require 相对模块** ⇒ preload 整体加载失败 ⇒ `window.codex` 不存在 ⇒ 渲染层首个 codex 调用
  （voiceSettingsGet）炸「界面发生错误」白屏（09-23 深夜事故，当晚修复）。守卫【2】有
  「preload.ts 无运行时相对 import」+「废弃文件不许回来」两条断言钉死。
  **加/改接口的新流程**：main handler → manifest 加一条 → `npm run gen:ipc` → ipc-registry 登记（文档账本）。
  ⛔ 生成物**勿手改** —— 守卫【2】新增 5 条逐字节比对，手改 / 忘跑 gen:ipc 都会红。
  ⛔ 64 个**多行/带逻辑**的 invoke 成员（`.then`、条件分支等）无法用一张表描述，仍留在 preload.ts 手写区；
  preload 是**自包含单文件**；守卫【2】的 preload 面 = `preload.ts`（`_ctx.mjs`）。
  ⛔ 提取/生成的三个坑（09-23 实测各踩一次）：① 成员前导注释与方法之间没逗号 ⇒ 同属一个 chunk，必须剥注释
  且**剥完再 trim**（否则 rest 以 `\n  名字:` 开头，名字正则 `^([\w$]+)` 失配 —— 恰好只有带注释的成员全挂）；
  ② d.ts 重名声明**首见优先**并打印告警（saveIdentity 一宽一窄，取窄的会让 identity_onboard 的 8 字段
  调用点爆 excess-property —— 提取当天就靠 tsc -b 抓出来了）；③ 工作树是 CRLF，按 `\n` 做 replace 会
  空转还报成功（includes 命中 ≠ replace 生效，要用长度差核对）。
  **顺带修掉的历史欠账**（提取器当场暴露）：10 个方法 preload 有、d.ts 没签名（渲染层从不跑 tsc ⇒ 静默），
  已补签名；2 个死方法 deleteSshServer / setSshServerEnabled（单数版 0 使用 + 与复数版共用 ssh:delete
  却发不同载荷形状）已删；theme:apply 在 ipc-registry 漏登记已补。
  验证：verify 逐字段一致；运行时探针（stub electron → 加载 dist-electron 产物 → 真调全部 251 方法比对
  channel + 3 项参数转发抽查）**254/254**；`npm run build`（tsc -b + vite）全绿；预检 19 ✗ = 环境基线。

- **钉顶不与折叠/流式互抢视口（09-23 深夜，守卫【126】）**：过程段自动折叠是 ~240ms 的
  `grid-template-rows` 过渡动画，**动画每帧都让内容变矮一点** —— 若 `pin-shrink-follow`
  在流式期间逐帧追这个收缩，会和正在增长的文字互抢视口。真机打点实录：一次回合里视口被
  拖上去两轮（top 2461→2270 / 2684→2472）再被流式推回来（gap 137→44→-110→-520），
  即用户报的「开始思考完在下，下个输出出来又去上面了」。修法两条：
  ① `pin-shrink-follow` 加**流式静默门**（`lastRevealAt`：1.2s 内有 packet-reveal 就不追）
  —— 折叠收缩在流式期间是暂态，文字马上把缺口长回来，追了必抖；流式停了的折叠
  照旧一次性贴合（transitionend → update 接住，09-23「折叠后缝隙要消」的诉求不丢）。
  ② `pin-fix` 的迟到 rAF 加**交棒守卫**（gap 锁定即退出）—— 排队期间内容长出视口后，
  迟到的纠偏会与 `pad-shrink` 逐帧互踹（实测 pad 一瞬间 111→147→111 翻烙饼）。
  ⛔ 这两个门的判据都来自 `__adbg` 打点（真机回合 595 条采样），别凭读代码推翻。

- **折叠芯片「真实状态映射」（09-23 深夜，守卫【127】）**：用户两连问——「正文中间怎么全是运行命令，
  编辑文件、读取、检索没有嘛」+ 附 WorkBuddy 截图「这种效果我也要，真实状态映射」。三件事：
  ① **命令二级意图** `commandIntentOf`（`command-display.mjs`，词法判定）：从命令字面认出
  modify/search/read/command 四档（Codex 引擎里读/搜/改文件大多也走 shell 命令，原先
  commandExecution 一律「运行命令」⇒ 芯片清一色同义词）。优先级 modify > search > read；
  认不出归 command，绝不硬猜。② **fileChange 行数映射**：摘要尾巴带聚合 `+N -M`
  （复用 `lib/diff-stats.ts` —— 与明细卡同一真相源，渲染层归一的 changes[].diff 是
  unified diff）；modify 词表改「编辑」。③ **运行态思考归段**：`buildSegments` 的
  思考直播判定带**事件顺序兜底** —— 上游经常不给 reasoning 回传 completed ⇒ 只看
  status/duration 会把已结束的思考永远留在正文外面；规则 = 后面已出现工具/正文 ⇒ 必然
  已结束、进折叠段（与 SessionQueue 的 reasoningActive 同一条规则，改一边必须同步另一边）；
  `turnFinished` 时不存在直播思考。⛔ **顺带修的真 bug**：新引擎运行时的启动器是**逗号形态**
  （`…\pwsh.exe,-Command "…"`），stripShellLauncher 原先不认 ⇒ 剥壳失效 + commandTarget 把
  pwsh 路径当文件目标。验证：真实 profile 33 条命令分布 定位 21/查看 11/运行 1（不再清一色）；
  vite ssrLoadModule 跑真 TS 断言 归段 6/6、剥壳 5/5。

- **命令行卡表头映射（09-24，紧随【127】；用户截图：「这个还是显示已运行没有显示具体的真实映射」）**：
  折叠里的命令卡表头原本是写死的「已运行 + 脚本原文」。现在：
  动词走 `INTENT_VERB`（`command-display.mjs`：查看/定位/编辑/运行 × 正在/已/失败/等待批准）；
  表头串 = **用途 · 目标** —— 用途 = `commandPurpose`（丢变量赋值段 → 优先取含中文的字符串
  字面量，脚本作者自己的输出标签就是用途；提炼不出退回剥壳截断原文，**绝不硬编假标签**），
  目标 = `commandTarget`。真机例：`$ws='D:\…\memory'; "=== 结构 ==="; Get-ChildItem -Recurse`
  → 「已定位 结构」；日志那条 → 「已查看 捕获链日志（末 8 行） · ud/memory-capture-debug.log」。
  ⛔ 顺带修两处脏形态：目标提取的扩展名字符集不含 `-` 且无尾部负向断言 ⇒ `.codex-harness`
  被截成半截 `.codex`（加 `(?![A-Za-z0-9_-])` + ext 允许 `-`）；`shortenPath` 不压多分隔符
  ⇒ 界面出现 `D://x//y`（现在 `//{2,}` 压成 `/`）。守卫【127】共 14 条钉住（含「不许退回
  写死的已运行」这条反向绊线）。展开里的完整命令仍保留引擎原文（要能照着复现）。

- **技能归属映射（09-24，守卫【127】；用户：「还有使用了技能类似没看见」）**：技能被使用时，
  引擎侧就是一条普通的 `mcpToolCall`（`nuphus/desktop_screenshot`）、`dynamicToolCall` 或
  `commandExecution`（markitdown …），界面上只显示「调用 server/tool」—— 看不出在用技能、
  更看不出用哪个。新增纯函数库 `src/lib/tool-display.mjs`：`skillOfItem(item)` 按工具名/服务器名/
  命令特征认出属于哪个内置技能（desktop_* / nuphus → 桌面自动化；browser_* / playwright /
  CloakBrowser → 浏览器自动化；markitdown / openpyxl → 文档转换；humanizer → 去 AI 味；
  no-ai-slop → 去 AI 套话；adhd → ADHD 写法；self-review → 自我审查；memory-distill → 记忆蒸馏；
  skill-authoring → 技能编写），**认不出返回 null**（保持原样，绝不硬贴标签）。
  落地两处：工具卡表头 → 动词「正在使用技能 / 已使用技能」+ **中文技能名**；折叠原子 →
  新增 `skill` 档（词表「使用技能 {t}」），芯片出「使用技能 桌面自动化」。
  ⛔ 判据是字面特征匹配，**不读盘、不读 SKILL.md**（技能目录用户可随时增删，渲染层不做 I/O）。
  ⚠️ 局限：技能若只被**读进提示词**（没有产生任何工具调用 / 命令），引擎不会上报 item ⇒
  界面无从显示 —— 这不是渲染层能补的，别为它加启发式。

- **技能/插件识别的两条路 + 「技能可能根本没有 item」（09-24，守卫【127】25 条）**：用户报「会话里用了
  好多技能，运行途中没有技能类展示」。做完的三件事与取证结论：
  ① **工具特征**（`desktop_*`/`browser_*`/`playwright`/`markitdown`/`nuphus`/humanizer…）→ 中文技能名；
  ② **目录路径兜底** `SKILL_PATH_RE`（`…/skills/<名>/…`）—— 用户自建技能、模型直接跑技能脚本的形态，
  名字不在内置表就**显示目录名本身**（别硬翻中文）；命中后若 `commandTarget` 取到的目标仍在技能目录里
  ⇒ **不显示**（否则芯片变成「技能名 · …/skills/技能名/scripts/x.py」重复噪音）；
  ③ **插件包名** `pluginOfItem`：`plugins/cache/<vendor>/<plugin>/…` 要取**插件名**不是 vendor
  （第一版取错成 `workbuddy-builtin`，等于没信息）；工具卡在工具名后追加「（插件 xxx）」。
  ⛔⛔ **取证结论（别再凭猜）**：在可访问的真实 userData（`%APPDATA%\Codex Harness Desktop\codex-home\sessions`）
  全量扫过 —— **没有任何一条 item 引用 `skills/<名>` 路径**，`payload.type` 里也**没有 skill 类事件**，
  `function_call` 只有 wait/write_stdin/view_image/request_user_input。所以：技能若只是**被注入上下文
  （读 SKILL.md）**使用，引擎**不产生任何 item** ⇒ 渲染层无从显示，这不是映射表能补的；想显示只能
  从**主进程下发侧**（thread/start 时的 dynamicTools / 会话启用了哪些技能）取真相源 —— 属新功能，先问再做。
  另一个常见坑：用户报的会话 ID 可能**不在本机任何可访问 userData 里**（多实例 / 改过 userData 路径 /
  `CODEX_HARNESS_USER_DATA` 隔离开关），先 `grep -rl <thread_id>` 全盘确认，找不到就直说，不要拿别的
  会话的数据当它的证据。

- **⛔⛔ 技能展示的关键前提：技能多半是「一条命令」（09-24 真回合取证，守卫【127】）**：用户屡次报
  「运行途中看不见用了哪个技能，只有模型自己说才知道」。真回合实锤（e2e profile，让模型读技能文件）：
  模型"用技能"产生的 item 是 **`CommandExecution`**（`Get-Content -LiteralPath '…/skills/humanizer/SKILL.md'`），
  不是 mcpToolCall / dynamicToolCall。**所以只把技能映射接到 ItemView（工具卡）是不够的 —— 命令行卡
  `TerminalPanel` 才是最常见的落点**；它原先只按 `commandIntentOf` 显示「已查看 …/skills/…」，
  看不出那是技能。现在命令行卡命中 `skillOfItem` ⇒ 动词改「正在/已使用技能」「使用技能失败」+ 中文技能名。
  ⛔ 目标显示规则抽成 `skillTargetLabel`（tool-display.mjs，卡片与折叠芯片**同一真相源**）：
  技能目录内的目标丢弃（读 SKILL.md 本身就是"用技能"，再拼文件名是重复）／长路径只留文件名／
  被截断成半截（`…/.workbud…`）丢弃／纯点目录段（`.workbuddy`，commandTarget 会误当扩展名）丢弃；
  没有目标时显示技能中文名，**不许退回命令原文**。
  真机结果（同一探针改前改后对照）：改前 `已查看 …/codex-home/skills/humanizer/SKILL.md`；
  改后 **`已使用技能 | 去 AI 味`**（含失败态 `使用技能失败 | 去 AI 味`），芯片同步出「使用技能 去 AI 味」。

- **技能要看得懂：本地技能清单是唯一翻译源（09-24，守卫【127】32 条）**：路径兜底能认出
  `…/skills/<名>/…`，但拿到的只是**目录名**（`electron-main-surgery`）—— 用户看不懂。真相源
  = 主进程 `skills:local-list`（`LocalSkillEntry` 带 descriptionZh / description / folder），启动
  拉一次注册进纯函数表 `registerKnownSkills`（tool-display.mjs）：路径命中 ⇒ 显示中文/描述，
  未注册则退回目录名，**取不到也不影响功能**（不报错、不重试）。⛔ 渲染层仍然**不读盘**。
  证据面同时放宽：工具**入参**（arguments）里的技能路径也算（限量 2000 字符）；但**正文/思考
  不算证据**（`agentMessage` 里提技能名不能标成"用了技能"），只对 commandExecution /
  mcpToolCall / dynamicToolCall 判定。
  ⛔ 目标显示的四条丢弃规则（同一真相源 `skillTargetLabel`，卡片与芯片共用）：技能目录内丢弃
  （读 SKILL.md 就是"用技能"，再拼文件名是重复）／半截路径（`…/.workbud…`）丢弃／**纯点目录段
  （`.workbuddy`）丢弃**（commandTarget 会把它误当扩展名先命中 —— 真机抓到过显示成
  「已使用技能 | .workbuddy」）／有文件名就只留文件名。

## ⚙️ 2026-09-24 IPC 强化 + 主题注册表（守卫【2】组新增 8 条）

**IPC 强化层（`electron/preload.ts` 手写区，生成段全部经过它）**：
- 生成模板改为 `__ipc(channel, minArgs, [args])`（`scripts/lib/gen-ipc-core.mjs`）。三件事：
  ① **arity 前置校验**：`minArgsOf(paramsImpl)` 顶层逗号计数 + 认两种可选形态（`p = 默认值`
     与 TS 可选 `p?: type`）。⛔ **只认 `=` 会把 `remoteQrcode(botId?: string)` 算成必选 ⇒
     把合法的零参调用拒掉**（实测踩到）；判定方向故意偏松（回调类型里的 `?:` 误判为可选只会少校验）。
  ② **错误归一化** `normalizeIpcError`：`No handler registered` → `ERR_NO_HANDLER`、
     `could not be cloned` → `ERR_UNCLONABLE`、`Error processing argument at index` → `ERR_BAD_ARGS`、
     超时 → `ERR_TIMEOUT`、其余 `ERR_INVOKE_FAILED`；错误对象带 `.code/.channel/.cause`
     （类型见 `vite-env.d.ts` 的 `IpcError`）。
  ③ **通道级超时表** `IPC_TIMEOUT_MS`（默认空 = 永不超时，**零行为变化**）—— 需要超时的通道加一行；
     不默认全局开是因为部分 invoke 等用户交互（对话框/审批），会误砍。
- **`__on(channel, listener, adapt)`**：事件订阅按 `(通道, 监听函数引用)` 幂等 + 精确退订。
  原 2 处 `removeAllListeners` 会清掉同通道别人的订阅（真 bug），已改掉；守卫**剥离注释后**断言
  preload 里不许再出现它。15 处标准订阅已迁移；`onEngineRestartDeferred`（一函数订阅两通道）保持原样。
- **扩展路径**：新接口 = main handler → manifest 加一条 → `npm run gen:ipc`（自动获得上面三项）。
  ⛔ `gen-ipc-bridge.mjs verify` 的解析器要能认新形态（`__ipc` 与旧 `ipcRenderer.invoke` 双兼容），
  改模板必须同轮改它，否则 verify 会红。
- ⛔ 批量改 preload 后**必须用「成员名集合」比对做守恒判据**：`git show HEAD:electron/preload.ts`
  与新文件各抽顶层成员名，断言集合相等（本次 335 → 335、零丢失）。文本 diff 看不出"少一个成员"。

**主题注册表（`src/lib/themes.ts`）**：`THEMES`（id/label/preview）+ `normalizeThemeId` 兜底
localStorage 脏值。外观页由注册表 `map` 渲染，**新增主题 = 表里加一行 + styles 加一段
`:root[data-theme="<id>"]` 变量块**（配色本体仍在 CSS，是唯一真相源）。主进程 `theme:apply`
只有"暗/不暗"二元，新增暗色系主题时才需同步改。

## 手写 invoke 全量收敛（09-24 上午，守卫【2】再扩 5 条）

- **64 条手写 invoke 迁进 manifest**（251 → 315 条）⇒ 全量 IPC 都获得 __ipc 的
  参数校验 / 错误归一化 / 超时表；preload 暴露面 **335 成员守恒**（迁前后同为 335）。
  剩 1 条保留手写：`voiceAudio` 走 `ipcRenderer.send`（fire-and-forget，无返回，不适用 invoke 包装）。
- ⛔ 迁移三坑（都写进守卫/脚本）：
  ① **d.ts 声明删除要按「含 {}[]<> 的括号栈」扫到分号** —— 我只算圆括号时，返回类型里的
  `Promise<{ ready: boolean; … }>` 的内层 `;` 会提前截断，把 d.ts 删成语法错误（实测踩到，回滚重做）。
  ② **returns 的权威来源是 d.ts 声明**（比 preload 的 `as` 更精确，且同名可能两处：详细版 +
  `Promise<any>` 宽松版）—— 拿 cast 当 returns 会让类型精度退化。
  ③ **manifest 的 `count` 字段要跟着 channels 走** —— 忘了会红在【2】；现在 `gen:ipc generate`
  自动同步（不再靠人记）。
- **真机发现的一个硬事实（重要）**：`contextBridge` 回传错误时**自定义属性会丢** ——
  渲染层 `catch (e) => e.code` 恒为 `undefined`（结构化克隆只留 message/stack）。所以：
  错误码同时写进**消息前缀** `[ERR_XXX] <channel>: …`，并新增渲染层解析入口
  `src/lib/ipc-error.mjs`（`ipcErrorCodeOf` / `ipcErrorChannelOf` / `ipcErrorHintOf`）。
  ⛔ 以后写 IPC 错误处理**别读 e.code**，读 message 前缀（守卫已钉）。
- 验证：真机冒烟 10/10（应用起、成员 335、4 个迁移方法真实往返、错误前缀可解析）、
  离线探针 11/11、gen check+verify 315 通过、tsc 无诊断、预检【2】21 条全绿、总 19 ✗ = 环境基线。

## 🧭 设置页「拓展接口」（09-24，守卫【128】）

用户要求：「设置界面加一个拓展接口菜单，集中管理所有可拓展的接口，分类展示，每个接口给出用途 /
可拓展位置 / 拓展方式 / 生效方式 / 配套说明」。

- **数据真相源 = `src/lib/extensibility-catalog.mjs`**（纯函数，12 条 / 4 组：接口与契约 ·
  界面与呈现 · 智能体与能力 · 记忆与质量）。⛔ 页面只渲染、**不许把清单写进组件**（守卫钉）。
  每条五要素齐全（用途 / 位置 / 步骤 / 生效 / 配套守护+坑），字段缺项会被【128】指出。
- 页面 `src/features/settings-extensibility/`（lazy 注册进 `00-settings-registry.tsx` + `settingsNav`
  的「开发工具」组 + `SettingsPage` 联合类型 + `HelpDialog` 的 OVERVIEW_GROUPS —— **四处都要改**，
  少 `OVERVIEW_GROUPS` 会红在【32】「总览漏了这些设置页」）。
- 交互：分组折叠 + 搜索过滤 + 每条一键复制为 Markdown（贴同事/喂 AI 都能照做）+ 顶部统计
  （IPC 通道数 / 主题数 / 本地技能数 / 拓展点数，技能数运行时现取）。
- ⛔ **统计数字由守卫保证不过期**：`IPC_CHANNEL_COUNT` 必须等于 manifest 实际条数（【128】）；
  docs 里曾长期写着「251 个通道」，迁移到 315 后没人同步 —— 数字写死在文档里必然会烂。

**同轮修掉的两个真实维护坑**：
1. **「生成段与 manifest 不一致」可能是行尾假红**：git（autocrlf）checkout 出来是 CRLF、生成器写 LF
   ⇒ 只要被 git 恢复过又没重跑生成，【2】就报红（`git diff --numstat` 是空的 = 纯行尾）。
   修法：生成器写入**跟随文件原有行尾**（`withEol`），生成器/守卫/verify 比对**一律归一化**（`normalizeEol`）。
2. **`gen:ipc` 现在会体检账本登记**：新增 channel 除了 manifest 还要在 `ipc-registry.ts` 记账，
   否则只有预检才报红 —— 现在生成时直接打印缺哪些 + 可粘贴的条目片段（不自动改手写表）。

## 🪟 任务栏图标 = AUMID 配对问题（09-24，守卫【2】新增 4 条）

用户报「任务栏的图标又变成默认的（Electron 原子图标）」。**根因不是图标文件坏了**（实测 build/icon.ico
是合法 ICO、6 个尺寸，窗口图标锚 app.getAppPath() 也正确），而是 **Windows 任务栏的图标来源规则**：

- Windows 靠 **AUMID（AppUserModelID）** 把"运行中的进程"与"某个注册了同一 AUMID 的快捷方式"配对；
  配对成功 ⇒ 任务栏取该快捷方式的图标（我们的 build/icon.ico）；配对失败 ⇒ 回退进程 exe 的图标。
- 用户是 **dev 形态**启动（桌面快捷方式 target = node_modules/electron/dist/electron.exe，工作目录 = 仓库根），
  exe 内嵌的是 Electron 原子图标 ⇒ 配对一旦失败，任务栏就是原子图标。
- 实测快捷方式里**已注册** AUMID `com.codexharness.desktop.dev`；而 main.ts 只在 app.isPackaged 时
  setAppUserModelId("com.codexharness.desktop") ⇒ **dev 下进程根本没有 AUMID，与快捷方式配不上对**。
- 旧注释写「dev 设置 AUMID 会顶掉窗口图标」—— 那是因为当时设的 id 没有对应快捷方式注册；
  现在 dev 快捷方式已注册 .dev，条件变了。

修法：`app.setAppUserModelId(app.isPackaged ? "com.codexharness.desktop" : "com.codexharness.desktop.dev")`
—— dev 与打包用两个不同 id，各自与自己的快捷方式对应。
⛔ 换机 / 重建快捷方式时必须给 lnk 带上 AUMID `com.codexharness.desktop.dev`（Windows 属性面板没有这个入口，
需 COM/IPropertyStore 或复制现有 lnk），否则 dev 任务栏会退回原子图标。
⛔ 不要退回 `if (app.isPackaged)` 条件（守卫【2】4 条钉住：AUMID 分叉 / .dev 常量 / ico 合法 / 图标路径锚 getAppPath）。

**改了两个仓库**（用户跑的是主仓库 D:\Codex Harness Desktop，不是副本）：主仓库 electron/main.ts 同款修改
+ npm run build:electron 重建（dev 跑的是 dist-electron/main.js，只改源码不生效 —— 产物 10:30、内含两个 AUMID）。
冒烟：隔离 userData 起实例 30 秒稳定运行，真实 codex-home 零写入。

- **截图（两种模式，各自全局快捷键）（09-24，守卫【129】）**：`electron/screenshot.ts`。
  · **输入框刻意不加截图图标按钮**（用户原话：「截图在输入框里面预提醒就行了，图标就不用加了」）——
    入口只有两条全局快捷键 + 设置页的「试试截图」；结果一律**放进输入框草稿**（图片 chip），
    `insertComposerImages` 是唯一落点，不自动发送。
  · `full` = 隐藏窗口后截整屏；`region` = 隐藏窗口 → 抓一帧**冻结画面** → 铺满该显示器的框选
    覆盖层 → 按选区裁剪。⛔ 用冻结帧而不是透明覆盖层：透明方案的合成行为在 Windows/macOS
    不一致（拖框重影、任务栏闪烁、mac 全屏空间异常），冻结帧的坐标可直接用于裁剪且不怕别的窗口刷新。
  · 框选覆盖层 = 一个 data: URL 的置顶 BrowserWindow + **独立 preload**（`electron/overlay-preload.ts`
    → `dist-electron/overlay-preload.js`，只开 `done/cancel` 两通道，不挂主桥）。⛔ 该 preload 必须
    自包含单文件——沙箱 preload 不许 require 相对模块（白屏事故同源），【129】有断言。
  · 快捷键：`createHotkeyRegistry`（两种模式各一条）+ `electron/accelerator.ts` 净化。
    ⛔ **先注册成功才注销旧键**（反过来新键被占用就两头空）；⛔ 两条不许同键（后者必然注册失败且原因不直观）；
    注册失败**照实回传**给设置页（`settings.errors`）——系统级抢占静默失败 = 用户以为功能坏了。
    默认 `CommandOrControl+Shift+Alt+A`（全屏，启用）/ `CommandOrControl+Shift+Alt+S`（框选，默认关）：
...
- **收藏夹（09-24，守卫【129】）**：`electron/favorites.ts` + `features/screenshot-favorites-ipc.ts`。
  ...
  · ⛔ **删除只认显式 id 列表**（`deleteFavorites(userData, ids)`），空数组 = 什么都不做；
    「清空全部」是独立动作 + 二次确认 —— 收藏是用户手攒的、删了没法从会话复原。
  · **一键发送**走 `bag.pendingCommandTextRef` 旁路（那是 `send()` 自己设计的「外部供给文本」通道）：
    ⛔ 绝不能「先 setPrompt 再 send」——`send()` 读的是**上一次渲染**的 `bag.prompt`，会发出上一条草稿；
    输入框已有内容时**不自动发送**，改成插入 + 明确提示（不覆盖用户正在写的东西）。
  · **加入 Agent 记忆**：`favorites:to-memory` 复用 `memoryLayers`（不自己拼层文件），scope =
    user/项目/背景/纪律；⛔ **先读后写**（层文件是整份覆盖语义，不读就写 = 抹掉用户已有记忆）；
    ⛔ 单行限长 300 字（记忆注入是硬预算，而收藏可以无限追加）。
  · 消息操作条只表达意图（`src/lib/favorite-bridge.ts` 的 `requestFavorite`）——`MessageFooter` 被
    ItemView/TurnView/SessionQueue 共用，加 prop 要穿 3~4 层无关组件（与语音那套 handler 注册同款做法）。
- **「设置页加一页」现在有 4 处要改**（`SharedPage` 类型 / 注册表 / `settingsNav` / 帮助总览）——
  【101】【32】【129】分别盯类型、导航、总览，漏一处就红。09-24 新增的两页（截图 / 收藏夹）走的就是这条路。
- **历史会话搜索（09-24）**：顶栏通知中心入口（铃铛）整块换成 🔍 搜索按钮，点开搜**全部历史会话的对话内容**。
  · 真相源 = rollout 原档（`codex-home/sessions/**` + `archived_sessions/**`），`history:search`
  （`electron/features/history-search-ipc.ts`）复用 `thread-backup.ts` 的 `scanSessionFiles` /
  `parseRolloutMessages` —— 与备份/导出同口径。⛔ 不搜渲染层 `bag.threads`（只持有已加载回合，
  无回合会话/归档会话搜不到）。
  · 性能护栏：先对原文 `indexOf` 快速否决（不含词不解析）→ >32MB 文件跳过并计数返回 → 文件按
  mtime 倒序扫、凑够 `limit` 个会话且后续更旧就提前收工。
  · 渲染面板 portal 到 body（顶栏 z-index:30 层叠上下文坑，同【73】断言已改盯新面板）；
  点结果 `openThread`（归档会话打不开时 toast 兜底）；IPC 失败 toast 提示，**不与「真无结果」同形**。
  · 通知中心**状态机仍在**（part02 的 noticeCenter 等，通知照常入队持久化），只是没了顶栏入口——
  「删图标」是 UI 层动作，数据层未动。
- **截图编辑器（09-24 二轮：用户「截图完没有编辑功能，就只是消失了」）**：两种模式（全屏/框选）
  截完都先进覆盖层编辑器，确认后才出图 —— 交互与微信/QQ 截图同款。
  · 工具条：画笔/矩形/椭圆/箭头/文字/马赛克 × 5 色 × 撤销（栈深 12）× 重选 / ✔ 确认 / ✕ 取消；
    确认 = Enter 或双击或 ✔；未选工具时点选区外 = 重新框选。
  · ⛔ **分辨率**：ink 画布与底图同为**物理像素**（绘制坐标 ×scaleFactor），确认回传合成 PNG
    （底图选区 + 标注层 drawImage 合成）—— 底图**不许 resize 到逻辑尺寸**（高分屏糊一半），
    守卫【129】盯死（`!resize({ width, height`）。主进程优先用回传图，缺失才裁原帧兜底。
  · ⛔ **确认即复制**：Electron 44 已移除 `clipboard.writeImage` ⇒ 复用 clipboard-ipc 的
    `copyImageFileToClipboard`（ClipboardItem + Blob，抽成共享函数）；不要加回 writeImage。
  · 全屏模式默认整屏全选（直接回车即得图）；覆盖层 preload 桥 `done(rect, image)` 两通道不变。
  · **默认不隐藏应用窗口**（09-24 用户：万一要截应用界面；hideWindow 默认 false，要纯桌面去
    设置页勾选）—— ⚠️ 只对没存过设置的用户生效，设置文件里已落盘 true 的需自行取消勾选。
  · 冒烟教训：`evalInTarget` 的表达式被包成 `return (expr)` ⇒ **多语句必须包 IIFE**（否则语法错
    静默不执行、resolve(undefined) 假绿）；`window.dispatchEvent` 的 target 不是 Node，
    `bar.contains(e.target)` 会抛 ⇒ 事件派发到 ink 等具体元素。
  · ⛔ 2026-09-24 下午起宿主封锁「node 进程内嵌套 spawn」（一律 EBUSY，连 python 也一样）⇒
