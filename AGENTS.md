...
## 📐 架构改造计划（2026-09-21 立项，进行中：先读 `docs/REFACTOR-PLAN-2026-09-21.md`）
...
3. **IPC 三件套同轮同步**：`main.ts` handler ↔ `preload.ts` 桥接 ↔ `vite-env.d.ts` 声明（预检 69 处守卫）。
4. **断言跟着搬**：预检直接点名读 `src/App.tsx` **69 处**、`electron/main.ts` **40 处** —— 搬代码必须同轮把对应断言改到新路径（一次搬一块、一次改一组，禁批量替换）。
5. **判据**：搬组件时若需要新加 >5 个 props，说明它和 App 状态耦合仍太深 → 回退，等状态抽成域 hook 再搬。
6. 域目录数控制在 12–16 个；不引入全局 store（会**增加**耦合面）。
