...
- **图片灯箱「打开位置」+ 复制修复（09-10）**：点击消息/附件图片放大后工具栏新增「在文件夹中显示」（本地路径/ 双编码解码后调 ，http URL 改  打开链接）。复制图片根因修复：渲染层  自定义协议拿不到 blob，本地图片改走主进程  IPC——**Electron 44 已移除 **，主进程写图片剪贴板须  →  → （注意用  而非全局 DOM 版，Buffer 要转 Uint8Array 才满足 BlobPart）。图片源形态归一化见 （绝对路径 / harness-image:// 双编码 / http 三态）。
