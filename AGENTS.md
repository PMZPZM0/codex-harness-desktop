...
- **发送锚顶（09-12 傍晚，消灭「出字上下跳动」）**：发送消息后不再贴底，而是把新消息**钉在对话区顶部**（WorkBuddy 观感），回复向下展开；回复增长超过 40px（长满第一屏）自动无缝转贴底跟随；用户上滚立即解除。实现要点（`App.tsx` anchorTopRef/anchorTurnIdRef + `scroll-utils.ts` 新增 `scrollToOffsetInstant`）：①锚定滚动必须**瞬时**（`.timeline` 有 CSS scroll-behavior:smooth，`scrollTo({behavior:"auto"})` 也走动画，要临时置 `style.scrollBehavior="auto"`）；②退出判据用「回复**增长量**（相对锚定时刻 scrollHeight）」而不是 away——长消息在锚定瞬间就有巨大 away，用它会在发送瞬间误退回贴底；③确认（optimisticConfirmed）前 scrollHeight 的猛涨是真实回合挂载的布局跳变，只刷新基线不退出；④真实回合元素可能比确认信号晚一帧挂载，钉顶时按 `turn-<id>` 动态查元素；⑤新会话首条消息发送后 756px 的欢迎块（welcome-state）要等会话建立才卸载，属中间态。回归：`scripts/e2e/scenarios/send-anchor-top.mjs`（12 断言，真实引擎发长消息，欢迎块中间态豁免 + 确认后钉顶 + 无双显）。
