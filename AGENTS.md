...
1. **⛔ 只跑「与本次改动相关」的验收项，禁止每轮全量重跑（2026-09-13 用户严令）**。
   用户原话：「能不能不要再跑旧的测试了，不要浪费我token啊…只能测试最新改动，给你说了几百遍」。
   - 一轮改动 → `resources/tools/node/node.exe scripts/accept.mjs --only <相关 id>`（可多次 `--only`，或只跑一个 id）；
     **默认不许再跑全量**。全量 `accept.mjs` 只在三种情况下跑：**发版/里程碑前**、
     **改动横跨多个模块无法界定范围**、**用户明确要求**。跑之前先说明理由。
   - 判断范围的原则：改渲染层交互/滚动 → 跑对应渲染项（如 `send-anchor` / `switch-running` / `reveal-on-switch`）；
     只改主进程/引擎/打包 → **不跑 CDP 验收**（那些断言测不到），跑 `check` + 需要时补预检结构守卫；
     纯文档/注释 → 只跑 `check`。
   - 判据：**断言必须能因为这次改动而变红**，否则它这一轮就是纯浪费（时间 + token）。
