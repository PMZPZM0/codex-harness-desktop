...
- **内置音色预设（09-13，台湾腔小美 / 贾维斯风 AI 管家）**：`resources/voice-presets/`（参考 wav 16k 单声道 + presets.json，随包 extraResources `voice-presets/`）。音源 = 微软 Edge TTS 官方音色合成（zh-TW-HsiaoChenNeural / en-GB-RyanNeural）——合成音源无真人版权问题，可随包分发；参考文本随预设落盘，**一键建档免 ASR 校对**。IPC：`voice:preset-list` / `voice:preset-apply`（幂等：同名档案已存在直接复用）→ createProfile → 选用写 `tts.profileId`。入口在 设置→语音通话→我的音色 卡片（需先装音色克隆模型）。**顺带修了 mergeSettings 吞 profileId 的真 bug**（显式字段映射吞新字段同款）：此前选用克隆音色后设置不持久、通话一直用内置音色（试听因显式传 id 才正常）——voice-presets 验收项抓到。**注意**：音色克隆走 ZipVoice（按需安装，见 09-12 条）；预设合成走 Edge TTS ws 协议（Sec-MS-GEC 令牌 = windows file time 取整 5 分钟 + trusted token 的 SHA256 大写）。
