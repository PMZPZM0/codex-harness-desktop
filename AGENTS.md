...
- **增强按钮提示气泡（09-17，用户「输入内容后那个亮起来的功能，加个小气泡提醒，词库丰富个性一点」）**：
  ...
  `src/lib/enhance-hints.mjs` 的 18 条词库（随机且不与上一条重复）。节奏（用户指定）：
  **每 5 次成功发送为一个周期**，周期到了之后**在下一次输入时**展示，**6 秒**后自动消失；
  点气泡本体=开始优化，点右侧 ✕=只关提示（`stopPropagation`），点增强按钮也消费掉。
  **⛔ 两个真坑（都是实测踩出来的，不是推测）**：
  ① **不能"发送成功当场显示"** —— 那一刻输入框已清空、回合在跑，增强按钮本身
  （`prompt.trim() || hasEnhanceBackup`）根本不渲染，气泡**没有任何可依附的位置，等于永不出现**。
  所以计数只置 `enhanceHintDueRef`（pending），真正的展示挂在派生值
  `enhanceAnchorVisible = (prompt.trim() || hasEnhanceBackup) && !activeThreadRunning` 上
  —— **展示条件必须与被依附按钮的渲染条件逐字一致**，否则会在回合运行中白消费掉这次提示
  （气泡看不见、机会也丢了）。该 effect 放在 `activeThreadRunning` 定义**之后**（它在那之后才算出来）。
  ② **`.enhance-hint` 必须写 `width: max-content`** —— 绝对定位元素在 32px 宽的按钮容器里会
  shrink-to-fit，气泡被压成**竖排窄条**（一个字一行，完全不可读）。这一条**断言全绿但视觉是坏的**，
  是截图抓出来的：位置/动画/结构断言都通过，没人测"宽度够不够读"。**UI 改动必须看截图。**
  节奏逻辑抽成纯函数 `shouldShowHint(n)`，离线可测（真发 5 条消息要几分钟且污染会话历史）。
  预检【32】⑰ 七条守卫 + 9 条变异反证成立（含"注释里提到属性名把守卫顶成假绿"这条 ——
  守卫判 CSS 前必须先 `stripC` 去注释）。真启动验收 12 条全绿（真发一条消息走完整链路）。
...
- **⛔ 乐观气泡「安全阀」的判据（09-17 用户实测：「消息发出去，先是旧内容 + 生成条，过一会才看到我的消息」）**：
  ...
  并在**两处发送点**（fastArm / 慢路径）复位该 ref。修复后同一探针：气泡存活 1344ms、直到真实消息接管那一刻。
